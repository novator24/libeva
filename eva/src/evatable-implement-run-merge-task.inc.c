                                    ...
                                    GError   **error)
{
  ...
  for (;;)
    {
      n_written++;
      switch (eva_table_file_feed_entry (output,
...
#endif
          break;
        case EVA_TABLE_FEED_ENTRY_ERROR:
          return FALSE;
        default:
...
#endif
      eva_table_reader_advance (reader);
...
do_advance_a:
          ...
          eva_table_reader_advance (readers[0]);
          if (readers[0]->error)
            {
...
do_advance_a_and_b:
          ...
          eva_table_reader_advance (readers[0]);
          eva_table_reader_advance (readers[1]);
          if (readers[0]->error)
            {
              g_set_error (error, readers[0]->error->domain,
                           readers[0]->error->code,
