...
/* These functions provide an interface to the mechanism
   ...
   The use-cases fall into two categories:
     - the sending of file-descriptors is going on between
       ...
       call eva_pass_fd_make_pair() and ensure
...
#ifndef __EVA_PASS_FD_H_
#define __EVA_PASS_FD_H_

#include <glib.h>

G_BEGIN_DECLS

gboolean eva_pass_fd_make_pair    (int          *sender_fd_out,
                                   int          *receiver_fd_out,
				   GError      **error);
int      eva_pass_fd_make_sender  (GError      **error);
...
gboolean eva_pass_fd_sendto       (int           sender_fd,
                                   ...
                                   const guint8 *aux_info_data,
                                   GError      **error);
int      eva_pass_fd_receive      (int           receiver_fd,
