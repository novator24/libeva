                         ...
                         GError      **error)
{
  ...
  {
    char *hex = eva_escape_memory_hex (compressed_data + used, compressed_data_len - used);
  ...
  if (zrv != Z_OK)
    {
      g_set_error (error, EVA_G_ERROR_DOMAIN, EVA_ERROR_CORRUPT,
    ...
    if (entry->index == index)
      {
        if (entry->prev_lru != NULL)
          {
            EVA_LIST_REMOVE (GET_LRU_LIST (ffile), entry);
            EVA_LIST_PREPEND (GET_LRU_LIST (ffile), entry);
          }
        return entry;
      }
  ...
  if (ffile->cache_entries_count == ffile->max_cache_entries)
    {
      ...
      CacheEntry **pprev;
      EVA_LIST_REMOVE_LAST (GET_LRU_LIST (ffile));
                  ...
                  GError        **error)
{
  ...
  if (reader->file_size < MAX_MMAP)
    ...
    {
      reader->mmapped = NULL;
      eva_table_buffer_init (&reader->tmp_buf);
    }
  return TRUE;
}
                   ...
                   GError        **error)
{
  ...
  if (reader->mmapped)
  ...
  else
    {
      ...
      if (rv < 0)
        {
          g_set_error (error, EVA_G_ERROR_DOMAIN, EVA_ERROR_FILE_PREAD,
        ...
        {
          g_set_error (error, EVA_G_ERROR_DOMAIN, EVA_ERROR_PREMATURE_EOF,
...
mmap_reader_clear (MmapReader *reader)
{
  if (reader->mmapped)
    ...
    eva_table_buffer_clear (&reader->tmp_buf);
}
                     ...
                     GError    **error)
{
  ...
  eva_table_buffer_init (&writer->tmp_buf);
  return TRUE;
}
...
{
  munmap (writer->mmapped, MMAP_WRITER_SIZE);
  eva_table_buffer_clear (&writer->tmp_buf);
}
...
static voidpf my_mem_pool_alloc (voidpf opaque, uInt items, uInt size)
{
  ...
  return eva_mem_pool_alloc0 (&builder->compressor_allocator, items*size);
}
                   ...
                   gboolean         preowned_mempool)
{
  if (preowned_mempool)
    {
      if (builder->compressor_allocator.all_chunk_list != NULL)
        {
          eva_mem_pool_destruct (&builder->compressor_allocator);
  ...
  eva_mem_pool_construct_with_scratch_buf (&builder->compressor_allocator,
  ...
  eva_table_buffer_set_len (&builder->compressed, 0);
}
...
flat_file_builder_new (FlatFactory *factory)
{
  if (factory->recycled_builders)
  ...
  else
    {
      ...
      eva_table_buffer_init (&builder->input);
      eva_table_buffer_init (&builder->first_key);
      eva_table_buffer_init (&builder->last_key);
      eva_table_buffer_init (&builder->compressed);
      eva_table_buffer_init (&builder->uncompressed);
                 ...
                 FlatFileBuilder *builder)
{
  if (ffactory->n_recycled_builders == ffactory->max_recycled_builders)
    {
      eva_table_buffer_clear (&builder->input);
      eva_table_buffer_clear (&builder->first_key);
      eva_table_buffer_clear (&builder->last_key);
      eva_table_buffer_clear (&builder->compressed);
      eva_table_buffer_clear (&builder->uncompressed);
      eva_mem_pool_destruct (&builder->compressor_allocator);
...
open_3_files (FlatFile                 *file,
              ...
              GError                  **error)
{
  char fname_buf[EVA_TABLE_MAX_PATH];
  guint open_flags;
  ...
  for (f = 0; f < N_FILES; f++)
    {
      eva_table_mk_fname (fname_buf, dir, id, file_extensions[f]);
             ...
             const guint8    *data)
{
  ...
  eva_table_buffer_ensure_extra (&builder->compressed, len / 2 + 16);
...
retry_deflate:
      ...
      if (builder->compressor.avail_out == 0)
        {
          eva_table_buffer_ensure_extra (&builder->compressed,
...
do_compress_flush (FlatFileBuilder *builder)
{
  ...
  eva_table_buffer_ensure_extra (&builder->compressed, 6 + 10);
  ...
  for (;;)
    {
      ...
      eva_table_buffer_ensure_extra (&builder->compressed, 64);
...
#if DEBUG_DUMP_COMPRESSED_DATA
  {
    char *hex = eva_escape_memory_hex (builder->compressed.data, builder->compressed.len);
    g_message ("  compressed_data=%s", hex);
    g_free (hex);
  }
#endif
                       ...
                       GError                  **error)
{
  ...
  if (builder->has_last_key)
    {
      ...
      encoded_len += tmp;
      memcpy (eva_table_buffer_set_len (&builder->uncompressed, encoded_len),
              enc_buf, encoded_len);

      /* copy non-prefix portion of key */
      memcpy (eva_table_buffer_append (&builder->uncompressed, key_len - prefix_len),
  ...
  else
    {
      ...
      memcpy (eva_table_buffer_set_len (&builder->first_key,
                                        key_len), key_data, key_len);
      eva_table_buffer_set_len (&builder->uncompressed, 0);
    }
  ...
  memcpy (eva_table_buffer_append (&builder->uncompressed, encoded_len),
          enc_buf, encoded_len);
  ...
  if (builder->compressed.len >= ffactory->bytes_per_chunk)
    {
      if (!flush_to_files (builder, error))
        return EVA_TABLE_FEED_ENTRY_ERROR;
    ...
    {
      builder->has_last_key = TRUE;
      memcpy (eva_table_buffer_set_len (&builder->last_key,
                                        key_len), key_data, key_len);
    }
  return builder->has_last_key ? EVA_TABLE_FEED_ENTRY_WANT_MORE
                        ...
                        GError                  **error)
{
  ...
  /* write the number of records to the front */
  {
    ...
    if (pwrite_rv < 0)
      {
        g_set_error (error, EVA_G_ERROR_DOMAIN, EVA_ERROR_FILE_PWRITE,
  ...
  while (n > 1)
    {
      ...
      if (!do_pread (ffile, FILE_INDEX, mid * SIZEOF_INDEX_ENTRY + INDEX_HEADER_SIZE, SIZEOF_INDEX_ENTRY, index_entry_data, error))
        {
          eva_table_buffer_clear (&firstkey);
          return FALSE;
        }
      index_entry_deserialize (index_entry_data, &index_entry);

      /* read key */
      eva_table_buffer_set_len (&firstkey, index_entry.firstkeys_len);
      ...
      if (compare_rv < 0)
      ...
      else
        {
          ...
          memcpy (eva_table_buffer_set_len (&query_inout->value, record->value_len),
...
read_and_uncompress_chunk (FlatFileReader *freader)
{
  ...
  if (index_entry.firstkeys_len != 0
    ...
    {
      freader->base_reader.error = g_error_new (EVA_G_ERROR_DOMAIN,
                 ...
                 GError      **error)
{
  ...
  for (f = 0; f < N_FILES; f++)
    {
      char fname_buf[EVA_TABLE_MAX_PATH];
      eva_table_mk_fname (fname_buf, dir, file->id, file_extensions[f]);
                        ...
                        GError                  **error)
{
  ...
  if (FREAD (&ief_header, 8, 1, freader->fps[FILE_INDEX]) != 1)
    {
      g_set_error (error, EVA_G_ERROR_DOMAIN, EVA_ERROR_FILE_READ,
