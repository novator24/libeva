...
#define ALIGN(size)	        _EVA_MEM_POOL_ALIGN(size)
#define SLAB_GET_NEXT_PTR(slab) _EVA_MEM_POOL_SLAB_GET_NEXT_PTR(slab)
#define CHUNK_SIZE	        8192

/* --- Allocate-only Memory Pool --- */
/**
 * eva_mem_pool_construct:
...
/**
 * eva_mem_pool_construct_with_scratch_buf:
 * @pool: the mem-pool to initialize.
			   ...
			   const char     *str)
{
  ...
  if (str == NULL)
    return NULL;
  L = strlen (str) + 1;
  return memcpy (eva_mem_pool_alloc_unaligned (pool, L), str, L);
}

/**
 * eva_mem_pool_destruct:
