...
static gboolean signal_handler (gint signal_no, gpointer data)
{
  ...
  switch (test_num)
	...
	eva_main_loop_add_timer (info->main_loop,
	                         deliver_signal, info, NULL, 10, 10);
	return TRUE;
      case 1:
	eva_main_loop_quit (info->main_loop);
	return FALSE;
    }
...
int main (int argc, char **argv)
{
  ...
  eva_main_loop_add_signal (main_loop, signal_info.sig_no,
                            signal_handler, &signal_info, 
			    mark_got_destroyed);
  kill (getpid (), signal_info.sig_no);
  while (!main_loop->quit)
    eva_main_loop_run (main_loop, -1, NULL);
