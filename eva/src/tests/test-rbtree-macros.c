...
add_tree (TreeNode **ptop,
          guint      v)
{
  TreeNode *node = eva_mem_pool_fixed_alloc (&tree_node_pool);
  TreeNode *extant;
  node->value = v;
  EVA_RBTREE_INSERT (TREE(ptop), node, extant);
  if (extant == NULL)
    return FALSE;
  eva_mem_pool_fixed_free (&tree_node_pool, node);
  return TRUE;
}
...
{
  TreeNode *found;
  EVA_RBTREE_LOOKUP_COMPARATOR (TREE(ptop), v, COMPARE_INT_WITH_TREE_NODE, found);
  return found != NULL;
}
          ...
          guint      v)
{
  ...
  if (found == NULL)
    return FALSE;
  EVA_RBTREE_REMOVE (TREE(ptop), found);
  eva_mem_pool_fixed_free (&tree_node_pool, found);
  return TRUE;
}
  ...
  for (i = 0; i < n; i++)
    {
      ...
      EVA_RBCTREE_INSERT (RBCTREE (top), arr + i, conflict);
  ...
  for (i = 0; i < n; i++)
    {
      ...
      RBCTreeNode *n;
      guint tv;
      EVA_RBCTREE_LOOKUP_COMPARATOR (RBCTREE (top), v, COMPARE_INT_WITH_TREE_NODE, n);
      g_assert (n == arr + i);
      EVA_RBCTREE_GET_NODE_INDEX (RBCTREE (top), n, tv);
          ...
          if (mm == mod_shuffle[m])
            {
              ...
              EVA_RBCTREE_REMOVE (RBCTREE (top), arr + i);
              DUMP_RBCTREE (top);
              if (do_validate_counts)
                validate_counts (top);
            }
          ...
          for (v = dmm; v < n; v += mod)
            {
              ...
              EVA_RBCTREE_LOOKUP_COMPARATOR (RBCTREE (top), v, COMPARE_INT_WITH_TREE_NODE, n);
              g_assert (n->value == v);
              EVA_RBCTREE_GET_NODE_INDEX (RBCTREE (top), n, actual_index);
...
int main()
{
  ...
  EVA_RBTREE_FIRST (TREE(&tree), node);
  g_assert (node == NULL);
  EVA_RBTREE_LAST (TREE(&tree), node);
  g_assert (node == NULL);
  ...
  for (i = 0; i <= 999; i++)
    {
      EVA_RBTREE_SUPREMUM_COMPARATOR (TREE(&tree), i, COMPARE_INT_WITH_TREE_NODE, node);
      g_assert (node);
      g_assert (node->value == (i%2)?i:(i+1));
    }
  EVA_RBTREE_SUPREMUM_COMPARATOR (TREE(&tree), 1000, COMPARE_INT_WITH_TREE_NODE, node);
  g_assert (node==NULL);
  for (i = 1; i <= 1000; i++)
    {
      TreeNode *node;
      EVA_RBTREE_INFIMUM_COMPARATOR (TREE(&tree), i, COMPARE_INT_WITH_TREE_NODE, node);
      g_assert (node);
      g_assert (node->value == (i%2)?i:(i-1));
    }
  EVA_RBTREE_INFIMUM_COMPARATOR (TREE(&tree), 0, COMPARE_INT_WITH_TREE_NODE, node);
  g_assert (node==NULL);
  for (i = 1; i <= 999; i += 2)
