...
header_copy_through_buffers (GskHttpHeader *header)
{
  ...
  eva_buffer_construct (&buffer2);

  eva_http_header_to_buffer (header, &buffer1);
  len1 = buffer1.size;
  data1 = g_malloc (len1);
  eva_buffer_peek (&buffer1, data1, len1);

  header1 = eva_http_header_from_buffer (&buffer1, is_request, EVA_HTTP_PARSE_STRICT, &error);
  g_assert (header1);
  g_assert (buffer1.size == 0);

  eva_http_header_to_buffer (header1, &buffer2);
  len2 = buffer2.size;
  data2 = g_malloc (len2);
  eva_buffer_peek (&buffer2, data2, len2);

  header2 = eva_http_header_from_buffer (&buffer2, is_request, EVA_HTTP_PARSE_STRICT, &error);
  g_assert (header2);
  ...
  for (test = 0; test < 100; test++)
    {
      ...
      eva_buffer_append_foreign (&buffer, txt, len, g_free, txt);
      header = eva_http_header_from_buffer (&buffer, is_request, 0, NULL);
      if (header)
...
#define CGI_ASSERT(cond)                                        \
  ...
  table = eva_http_request_parse_cgi_query_string (query_string);
  ...
  /* --- list --- */
  va_start (args, first_key);
  list = eva_http_parse_cgi_query_string (query_string, NULL);
...
int main(int argc, char **argv)
{
  ...
  g_value_unset (&value);
  g_value_init (&value, EVA_TYPE_HTTP_CONNECTION);
  g_object_get_property (G_OBJECT (header1), "connection", &value);
  g_assert (g_value_get_enum (&value) == EVA_HTTP_CONNECTION_NONE);
  g_value_unset (&value);
  g_assert (headers_equal (header0, header1));
  g_assert (header1->http_major_version == 1
         && header1->http_minor_version == 0);
  g_assert (eva_http_header_get_connection (header1) == EVA_HTTP_CONNECTION_CLOSE);
  g_assert (eva_http_header_get_connection_type (header1) == EVA_HTTP_CONNECTION_NONE);
  ...
  g_assert (g_value_get_enum (&value) == EVA_HTTP_CONNECTION_NONE);
  g_value_unset (&value);
  g_value_init (&value, EVA_TYPE_HTTP_TRANSFER_ENCODING);
  g_object_get_property (G_OBJECT (header1), "transfer-encoding", &value);
  g_assert (g_value_get_enum (&value) == EVA_HTTP_TRANSFER_ENCODING_CHUNKED);
  g_value_unset (&value);
  g_assert (eva_http_header_get_connection (header1) == EVA_HTTP_CONNECTION_KEEPALIVE);
  g_assert (eva_http_header_get_connection_type (header1) == EVA_HTTP_CONNECTION_NONE);
  g_object_unref (header0);
  g_object_unref (header1);

  {
    ...
    header0 = header_from_string (TRUE,
				  ...
				  "\n");
    header1 = header_copy_through_buffers (header0);
    g_assert (EVA_IS_HTTP_REQUEST (header0));
    g_assert (headers_equal (header0, header1));
    request = EVA_HTTP_REQUEST (header1);
  ...
  /* test cache-control in request */
 {
    ...
    for (iter = 0; iter < 2; iter++)
      {
        ...
        g_assert (header->connection_type == EVA_HTTP_CONNECTION_CLOSE);
    ...
    for (iter = 0; iter < 2; iter++)
      {
        ...
        g_assert (eva_http_header_lookup_misc (header, "X-Cache") != NULL);
        g_assert (eva_http_header_lookup_misc (header, "X-Powered-By") != NULL);
        g_assert (strcmp (eva_http_header_lookup_misc (header, "X-Cache"), "HIT from eserver.org") == 0);
        g_assert (strcmp (eva_http_header_lookup_misc (header, "X-Powered-By"), "ASP.NET") == 0);
        g_assert (eva_http_header_lookup_misc (header, "PICS-Label") != NULL);
      }
    g_object_unref (header0);
    g_object_unref (header1);
  }
  ...
  /* example from rfc 2617, page 5 */
  {
    ...
    g_assert (res->authenticate->mode == EVA_HTTP_AUTH_MODE_BASIC);
  ...
  /* example from rfc 2617, page 6 */
  {
    ...
    g_assert (req->authorization->mode == EVA_HTTP_AUTH_MODE_BASIC);
