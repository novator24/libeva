...
int main (int argc, char **argv)
{
  ...
  if (!eva_xmlrpc_parser_feed (parser, 
			      "<?xml version=\"1.0\"?>\n"
			      "<methodCall>\n"
  ...
  request = eva_xmlrpc_parser_get_request (parser);
  response = eva_xmlrpc_parser_get_response (parser);
  g_assert (request != NULL);
  g_assert (response == NULL);
  g_assert (request->params->len == 1);
  g_assert (strcmp (request->method_name, "examples.getStateName") == 0);
  g_assert (request->params->values[0].type == EVA_XMLRPC_INT32);
  g_assert (request->params->values[0].data.v_int32 == 41);
  eva_xmlrpc_request_unref (request);
  request = eva_xmlrpc_parser_get_request (parser);
  g_assert (request == NULL);
  ...
  g_assert (response->params->values[0].type == EVA_XMLRPC_STRING);
  g_assert (strcmp (response->params->values[0].data.v_string, "South Dakota") == 0);
  eva_xmlrpc_response_unref (response);
  response = eva_xmlrpc_parser_get_response (parser);
  g_assert (response == NULL);
  ...
  g_assert (response->has_fault);
  g_assert (response->fault.type == EVA_XMLRPC_STRUCT);
  eva_xmlrpc_response_unref (response);
  response = eva_xmlrpc_parser_get_response (parser);
  g_assert (response == NULL);
  ...
  g_assert (response->params->values[0].type == EVA_XMLRPC_STRUCT);
  ...
  g_assert (response->params->values[0].data.v_struct->members[0].value.type == EVA_XMLRPC_ARRAY);
  array = response->params->values[0].data.v_struct->members[0].value.data.v_array;
  g_assert (array->len == 4);
  g_assert (array->values[0].type == EVA_XMLRPC_INT32);
  g_assert (array->values[0].data.v_int32 == 34);
  g_assert (array->values[1].type == EVA_XMLRPC_INT32);
  g_assert (array->values[1].data.v_int32 == 42);
  g_assert (array->values[2].type == EVA_XMLRPC_INT32);
  g_assert (array->values[2].data.v_int32 == 16);
  g_assert (array->values[3].type == EVA_XMLRPC_STRING);
  ...
  g_assert (response->params->values[0].data.v_struct->members[1].value.type == EVA_XMLRPC_STRUCT);
  ...
  {
    char rand_buf[100];
    request = eva_xmlrpc_request_new (NULL);

    eva_xmlrpc_request_add_int32 (request, 101);
    eva_xmlrpc_request_add_boolean (request, TRUE);
    eva_xmlrpc_request_add_double (request, 2.7182182);
    eva_xmlrpc_request_add_string (request, "hello world");
    eva_xmlrpc_request_add_date (request, 1066441969);
    {
	...
	((guint32 *) ba->data)[i] = rand();
      eva_xmlrpc_request_add_data (request, ba);
	...
	if (!eva_xmlrpc_parser_feed (parser, frag->buf + frag->buf_start, frag->buf_length, &error))
      ...
      g_assert (request->params->values[1].type == EVA_XMLRPC_BOOLEAN);
      g_assert (request->params->values[1].data.v_boolean == TRUE);
      g_assert (request->params->values[2].type == EVA_XMLRPC_DOUBLE);
      g_assert (2.7182181 <= request->params->values[2].data.v_double
	     && 2.7182183 >= request->params->values[2].data.v_double);
      g_assert (request->params->values[3].type == EVA_XMLRPC_STRING);
      g_assert (strcmp (request->params->values[3].data.v_string, "hello world") == 0);
      g_assert (request->params->values[4].type == EVA_XMLRPC_DATE);
      g_assert (request->params->values[4].data.v_date == 1066441969);

      g_assert (request->params->values[5].type == EVA_XMLRPC_BINARY_DATA);
      ...
      eva_xmlrpc_request_unref (request);
      g_assert (eva_xmlrpc_parser_get_request (parser) == NULL);
      g_assert (eva_xmlrpc_parser_get_response (parser) == NULL);
    }
  }
