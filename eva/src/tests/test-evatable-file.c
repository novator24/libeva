  ...
  for (i = start; i < end; i++)
    {
      GError *error = NULL;
      gen_kv (i, key, value);
      switch (eva_table_file_feed_entry (file, key->len, key->data, value->len, value->data, &error))
        {
        case EVA_TABLE_FEED_ENTRY_WANT_MORE:
        case EVA_TABLE_FEED_ENTRY_SUCCESS:
          break;
        default:
          g_error ("eva_table_file_feed_entry: %s", error->message);
        }
    }
               ...
               guint         step)
{
  ...
  for (i = start; i < end; i += step)
    {
      gen_kv (i, key, value);
      if (!eva_table_file_query (file, &query, &error))
        g_error ("eva_table_file_query: %s", error->message);
    }
  g_byte_array_free (key, TRUE);
  g_byte_array_free (value, TRUE);
  eva_table_file_query_clear (&query);
}
...
finish_file (EvaTableFile *file)
{
  gboolean ready;
  GError *error = NULL;
  if (!eva_table_file_done_feeding (file, &ready, &error))
    g_error ("eva_table_file_done_feeding: %s", error->message);
  while (!ready)
    if (!eva_table_file_build_file (file, &ready, &error))
      g_error ("eva_table_file_build_file: %s", error->message);
}
          ...
          guint64              id)
{
  ...
  /* --- small test --- */
  file = eva_table_file_factory_create_file (factory, dir, id, &hints, &error);
  if (file == NULL)
    g_error ("eva_table_file_factory_create_file: %s", error->message);
...

  {
    guint state_len;
    guint8 *state_data;
    if (!eva_table_file_get_build_state (file, &state_len, &state_data, &error))
      g_error ("eva_table_file_get_build_state: %s", error->message);
    if (!eva_table_file_destroy (file, dir, FALSE, &error))
      g_error ("eva_table_file_destroy: %s", error->message);
    file = eva_table_file_factory_open_building_file (factory, dir, id, state_len, state_data, &error);
    if (file == NULL)
      g_error ("eva_table_file_factory_open_building_file: %s", error->message);
    g_free (state_data);
  }
  ...
  if (!eva_table_file_destroy (file, dir, FALSE, &error))
    g_error ("eva_table_file_destroy: %s", error->message);
  file = eva_table_file_factory_open_file (factory, dir, id, &error);
  if (file == NULL)
    g_error ("eva_table_file_factory_open_building_file: %s", error->message);

  query_entries (file, gen_kv_0, 0, end, 1);

  if (!eva_table_file_destroy (file, dir, TRUE, &error))
    g_error ("eva_table_file_destroy: %s", error->message);
}
static void
     ...
     char **argv)
{
  ...
  factory = eva_table_file_factory_new_flat ();
