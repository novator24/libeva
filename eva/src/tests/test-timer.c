  ...
  if (rand() < RAND_MAX / 10)
    {
      ...
      eva_source_adjust_timer (info->source, info->period, info->period);
      info->intended_runtime = eva_main_loop_default()->current_time;
    }
...
make_random_source (void)
{
  ...
  info->source = eva_main_loop_add_timer (eva_main_loop_default(),
...
test_timer_2 (gpointer data)
{
  ...
  ct = &eva_main_loop_default ()->current_time;
...
int main (int argc, char **argv)
{
  ...
  test_2 ();
  last_runtime = eva_main_loop_default ()->current_time;
