#include "../mime/evamimemultipartencoder.h"
...
test_with_pieces (GSList *pieces)
{
  ...
  options[0] = "boundary";
  options[1] = EVA_MIME_MULTIPART_ENCODER_GOOD_BOUNDARY;
  options[2] = NULL;
  decoder = eva_mime_multipart_decoder_new (options);
  g_assert (decoder);
  eva_mime_multipart_decoder_set_mode (decoder, EVA_MIME_MULTIPART_DECODER_MODE_ALWAYS_MEMORY);

  encoder = eva_mime_multipart_encoder_new_defaults ();
  g_assert (encoder);

  g_assert (eva_stream_attach (EVA_STREAM (encoder),
			       EVA_STREAM (decoder),
			       NULL));
  ...
  eva_mime_multipart_decoder_trap (decoder, handle_new_piece_available,
				   handle_new_piece_shutdown,
				   &decoder_info, NULL);
  eva_mime_multipart_encoder_terminate (encoder);
  ...
  while (pieces != NULL)
		  ...
		  orig_piece->content_length) != 0)
	{
	  ...
	  g_error ("mismatched content in piece:\n"
		   ...
		   "  got='%s'\n"
		   "  orig encoding=%s\n",
		   eva_escape_memory (orig_piece->content_data, orig_piece->content_length),
		   eva_escape_memory (out_piece->content_data, out_piece->content_length),
      ...
      eva_mime_multipart_piece_unref (orig_piece);
      eva_mime_multipart_piece_unref (out_piece);
    }
...
make_random_piece (void)
{
  ...
  eva_mime_multipart_piece_set_transfer_encoding (piece, transfer_enc);
  ...
  /* Generate the random data. */
  {
	...
	do {
      ...
      for (i = 0; i < len; i++)
	slab[i] = (char) rand ();
    eva_mime_multipart_piece_set_data (piece, slab, len, g_free, slab);
  }
  ...
  if (rand () / (RAND_MAX/2))
      ...
      if (rand () / (RAND_MAX/2))
	{
      ...
      eva_mime_multipart_piece_set_type (piece, buf0, buf1, charset, NULL);
    }
  if (rand () / (RAND_MAX/2))
    {
      char buf[256];
      rand_nice_str (buf, sizeof (MAX_FIELD_LEN));
      eva_mime_multipart_piece_set_id (piece, buf);
    }
  if (rand () / (RAND_MAX/2))
    {
      char buf[256];
      rand_nice_str (buf, sizeof (MAX_FIELD_LEN));
      eva_mime_multipart_piece_set_description (piece, buf);
    }
  if (rand () / (RAND_MAX/2))
    {
      char buf[256];
      rand_nice_str (buf, sizeof (MAX_FIELD_LEN));
      eva_mime_multipart_piece_set_location (piece, buf);
    }
