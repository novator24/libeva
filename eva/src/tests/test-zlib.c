		        ...
		        gpointer                data)
{
  ...
  eva_buffer_peek (buffer, res->data, res->size);
}
...
test_with_string (const char *str)
{
  ...
  deflator = eva_zlib_deflator_new (-1, -1);
  inflator = eva_zlib_inflator_new ();
  memory_output = eva_memory_buffer_sink_new (memory_buffer_callback, &result, NULL);
  eva_stream_attach (memory_input, deflator, &error);
  g_assert (error == NULL);
  eva_stream_attach (deflator, inflator, &error);
  g_assert (error == NULL);
  eva_stream_attach (inflator, memory_output, &error);
  g_assert (error == NULL);
