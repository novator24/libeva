#include <string.h>
#include <ctype.h>
#include "evamimemultipartdecoder.h"
...
update_idle_notify_writeable (EvaMimeMultipartDecoder *multipart_decoder)
{
  ...
  if (multipart_decoder->feed_stream != NULL)
    is_writable = eva_io_is_polling_for_read (multipart_decoder->feed_stream);
  else
    is_writable = multipart_decoder->first_piece == NULL;
  //is_writable = eva_hook_get_last_poll_state (relevant_hook);
#if 0
  g_message ("update_idle_notify_writeable: (feed_stream=%p, feed_stream->read_hook: on: %s; first_piece=%p) => %u",
             ...
             is_writable);
#endif
  eva_io_set_idle_notify_write (multipart_decoder, is_writable);
}
...
eva_mime_multipart_decoder_shutdown_write (EvaIO *io, GError **error)
{
  ...
  if (decoder->n_pieces_alloced == decoder->n_pieces_obtained)
    {
      eva_hook_notify_shutdown (_EVA_MIME_MULTIPART_DECODER_HOOK (decoder));
    }
  if (decoder->state != STATE_ENDED)
    {
      eva_io_set_error (io, EVA_IO_ERROR_WRITE, EVA_ERROR_BAD_FORMAT,
		...
		guint                     piece_index)
{
  ...
  if (piece_index == decoder->next_piece_index_to_append)
	...
	eva_hook_mark_idle_notify (_EVA_MIME_MULTIPART_DECODER_HOOK (decoder));
    }
  else
    {
      if (decoder->piece_index_to_piece == NULL)
...
feed_buffer_into_feed_stream (EvaMimeMultipartDecoder *multipart_decoder)
{
  ...
  char *bdy_tmp;
  eva_buffer_iterator_construct (&iterator, &multipart_decoder->buffer);
  if (multipart_decoder->state == STATE_CONTENT_MIDLINE)
    {
      at_line_start = eva_buffer_iterator_find_char (&iterator, '\n');
      if (at_line_start)
	eva_buffer_iterator_skip (&iterator, 1);
      n_pending = eva_buffer_iterator_offset (&iterator);
    }
  ...
  while (at_line_start)
    {
      unsigned n_peeked = eva_buffer_iterator_peek (&iterator, bdy_tmp, multipart_decoder->boundary_str_len + 4);
      ...
      else
	{
	  ...
	  if (!eva_buffer_iterator_find_char (&iterator, '\n'))
	    {
	      multipart_decoder->state = STATE_CONTENT_LINE_START;
	      break;
	    }
	  ...
	  n_discard = eva_buffer_iterator_offset (&iterator) + 1 - n_pending;
	  break;
	}
    }
  ...
  if (n_pending > 0)
      ...
      if (n_pending >= 2)
	{
	  char tmp[2];
	  guint bufdisc = 2;
	  eva_buffer_transfer (eva_buffer_stream_peek_read_buffer (feed), &multipart_decoder->buffer, n_pending - 2);
	  eva_buffer_peek (&multipart_decoder->buffer, tmp, 2);
	  if (memcmp (tmp, "\r\n", 2) == 0)
	    multipart_decoder->swallowed_crlf = TRUE;
	  else if (tmp[1] == '\r')
	    {
	      g_assert (n_discard == 0);
	      eva_buffer_append (eva_buffer_stream_peek_read_buffer (feed), tmp, 1);
	    ...
	    {
	      eva_buffer_append (eva_buffer_stream_peek_read_buffer (feed), tmp, 2);
	      multipart_decoder->swallowed_crlf = FALSE;
	    }
	  eva_buffer_discard (&multipart_decoder->buffer, bufdisc);
	}
      else
	{
	  eva_buffer_transfer (eva_buffer_stream_peek_read_buffer (feed), &multipart_decoder->buffer, n_pending);
	  multipart_decoder->swallowed_crlf = FALSE;
	}
      eva_buffer_stream_read_buffer_changed (feed);
    }
  if (n_discard > 0)
    eva_buffer_discard (&multipart_decoder->buffer, n_discard);
   ...
   || multipart_decoder->state == STATE_ENDED)
    {
      ...
      // (If that was feed_stream, then eva_io_write_shutdown()
      // would work, be faster and cleaner)
      //eva_io_write_shutdown (multipart_decoder->feed_stream, NULL);
      eva_buffer_stream_read_shutdown (EVA_BUFFER_STREAM (multipart_decoder->feed_stream));
      g_object_unref (multipart_decoder->feed_stream);
      multipart_decoder->feed_stream = NULL;

      eva_mime_multipart_piece_unref (multipart_decoder->current_piece);
  ...
  if (terminated)
	...
	{
	  eva_hook_notify_shutdown (_EVA_MIME_MULTIPART_DECODER_HOOK (multipart_decoder));
	}
    }
  return TRUE;
}
		     ...
		     guint piece_index)
{
  ...
  rv->piece = eva_mime_multipart_piece_ref (piece);
...
handle_mime_piece_done (EvaBuffer *buffer, gpointer data)
{
  ...
  eva_buffer_peek (buffer, slab, buffer->size);
  eva_mime_multipart_piece_set_data (pd->piece, slab, size, g_free, slab);
...
piece_decoder_free (PieceDecoder *pd)
{
  ...
  eva_mime_multipart_piece_unref (pd->piece);
  g_free (pd);
}
...
done_header (EvaMimeMultipartDecoder *multipart_decoder)
{
  ...
  feed_stream = eva_buffer_stream_new ();
  multipart_decoder->feed_stream = EVA_STREAM (feed_stream);

  if (!eva_mime_make_transfer_encoding_decoders (transfer_encoding,
  ...
  if (!eva_stream_attach (EVA_STREAM (feed_stream), write_end, &error))
    {
  ...
  if (multipart_decoder->mode == EVA_MIME_MULTIPART_DECODER_MODE_ALWAYS_MEMORY)
    {
      ...
      sink = eva_memory_buffer_sink_new (handle_mime_piece_done,
					 data,
				         (GDestroyNotify) piece_decoder_free);
      if (!eva_stream_attach (read_end, sink, NULL))
	return FALSE;
      g_object_unref (sink);
    }
  else
...
#define IS_SEP_TYPE_CHAR(c) (isspace(c) || ((c)==';'))
      EVA_SKIP_CHAR_TYPE (at, IS_SUBTYPE_CHAR);
      ...
      for (;;)
	{
	  ...
	  const char *equals;
	  char *key, *value;
	  EVA_SKIP_CHAR_TYPE (at, IS_SEP_TYPE_CHAR);
	  if (*at == '\0')
	    break;
	  equals = at;
	  EVA_SKIP_CHAR_TYPE (equals, IS_SUBTYPE_CHAR);
	  if (*equals != '=')
	    {
	      g_set_error (error, EVA_G_ERROR_DOMAIN,
			   EVA_ERROR_BAD_FORMAT,
	  ...
	  if (*value_start == '"')
	    ...
	    {
	      EVA_SKIP_CHAR_TYPE (value_end, IS_SUBTYPE_CHAR);
			  ...
			  GError          **error)
{
  ...
  while (multipart_decoder->buffer.size != last_iter)
	...
	case STATE_WAITING_FOR_FIRST_SEP_LINE:
	  {
	    int index = eva_buffer_index_of (&multipart_decoder->buffer, '\n');
	    if (index < 0)
	      return;
            if ((guint)index+1 < sizeof (buf))
	      {
		eva_buffer_read (&multipart_decoder->buffer, buf, index + 1);
		buf[index] = 0;
		g_strchomp (buf);
		if (buf[0] != '-' || buf[1] != '-')
		  continue;
		...
		char *line = eva_buffer_read_line (&multipart_decoder->buffer);
		g_strchomp (line);
		if (line[0] != '-' || line[1] != '-')
		  continue;
...
eva_mime_multipart_decoder_init (EvaMimeMultipartDecoder *mime_multipart_decoder)
{
  ...
  EVA_HOOK_SET_FLAG (_EVA_MIME_MULTIPART_DECODER_HOOK (mime_multipart_decoder),
		     IS_AVAILABLE);
  eva_io_mark_is_writable (mime_multipart_decoder);
  eva_io_set_idle_notify_write (mime_multipart_decoder, TRUE);
}

static void
unref_piece_value (gpointer key, gpointer value, gpointer data)
{
  eva_mime_multipart_piece_unref (value);
}
...
/**
 * eva_mime_multipart_decoder_new:
...
/**
 * eva_mime_multipart_decoder_get_piece:
...
eva_mime_multipart_decoder_get_piece (EvaMimeMultipartDecoder *decoder)
{
  ...
  if (decoder->first_piece == NULL)
    {
      decoder->last_piece = NULL;
      eva_hook_clear_idle_notify (_EVA_MIME_MULTIPART_DECODER_HOOK (decoder));
      if (decoder->n_pieces_alloced == decoder->n_pieces_obtained
       && decoder->is_shutdown)
	{
	  eva_hook_notify_shutdown (_EVA_MIME_MULTIPART_DECODER_HOOK (decoder));
...
/**
 * eva_mime_multipart_decoder_set_mode:
