...
typedef enum
{
  EVA_URL_INTERPRETATION_RELATIVE, /* relative url */
...
eva_url_scheme_name (EvaUrlScheme scheme)
{
  switch (scheme)
    {
      case EVA_URL_SCHEME_FILE: return "file";
      case EVA_URL_SCHEME_HTTP: return "http";
      case EVA_URL_SCHEME_HTTPS: return "https";
      case EVA_URL_SCHEME_FTP: return "ftp";
      case EVA_URL_SCHEME_OTHER: return "?other?";
      default: return NULL;
    }
}
...
gboolean
eva_url_is_valid_hostname (const char *hostname, char *bad_char_out)
{
  while (*hostname)
    {
      if (!isalnum (*hostname)
...
gboolean
eva_url_is_valid_generic_component (const char *str, char *bad_char_out)
{
...
url_check_is_valid (EvaUrl *url, GError **error)
{
  ...
  if (url->path && !eva_url_is_valid_path (url->path, &bad_char))
    {
      g_set_error (error, EVA_G_ERROR_DOMAIN, EVA_ERROR_INVALID_ARGUMENT,
	           "URL %s constructed uses unallowed character '%c' (0x%02x)",
  ...
  if (url->query && !eva_url_is_valid_query (url->query, &bad_char))
    {
      g_set_error (error, EVA_G_ERROR_DOMAIN, EVA_ERROR_INVALID_ARGUMENT,
	           "URL %s constructed uses unallowed character '%c' (0x%02x)",
  ...
  if (url->fragment && !eva_url_is_valid_fragment (url->fragment, &bad_char))
    {
      g_set_error (error, EVA_G_ERROR_DOMAIN, EVA_ERROR_INVALID_ARGUMENT,
	           "URL %s constructed uses unallowed character '%c' (0x%02x)",
				   ...
				   GError           **error)
{
  ...
  if (scheme == EVA_URL_SCHEME_FILE)
    interpretation = EVA_URL_INTERPRETATION_ABSOLUTE;
  else
    switch (num_slashes)
      {
	case 0:
	  interpretation = EVA_URL_INTERPRETATION_RELATIVE;
	  break;
	case 1:
	  interpretation = EVA_URL_INTERPRETATION_ABSOLUTE;
	  break;
	case 2:
	   ...
	   */
	  interpretation = EVA_URL_INTERPRETATION_REMOTE;
	  break;
	case 3:
  ...
  switch (interpretation)
    {
      case EVA_URL_INTERPRETATION_REMOTE:
	...
	/* fall through to parse the host-specific part of the url */
      case EVA_URL_INTERPRETATION_RELATIVE:
      case EVA_URL_INTERPRETATION_ABSOLUTE:
        {
      ...
      case EVA_URL_INTERPRETATION_UNKNOWN:
        {
	  g_set_error (error, EVA_G_ERROR_DOMAIN, EVA_ERROR_BAD_FORMAT,
		       _("cannot guess how to interpret %s:%s"),
	  	       eva_url_scheme_name (scheme), start);
	  goto error;
	}
    }

  if (interpretation == EVA_URL_INTERPRETATION_REMOTE
  ...
  url = g_object_new (EVA_TYPE_URL, NULL);
  url->scheme = scheme;
  if (scheme == EVA_URL_SCHEME_OTHER)
					 ...
					 EvaUrlScheme   *scheme_out)
{
  static UrlSchemeTableEntry table[] = {
    /* MUST BE SORTED */
    { "file", EVA_URL_SCHEME_FILE },
    { "ftp", EVA_URL_SCHEME_FTP },
    { "http", EVA_URL_SCHEME_HTTP },
    { "https", EVA_URL_SCHEME_HTTPS },
  };
  int i;
				     ...
				     GError         **error)
{
  ...
  return eva_url_new_from_scheme_specific (scheme, spec, error);
}

/**
 * eva_url_new_in_context:
 ...
 * if there appears to be a scheme, use eva_url_new();
 * otherwise call eva_url_new_from_scheme_specific() directly.
 * Alternately, it may be easier just to call
 * eva_url_new_in_context() directly all the time.
 *
 * See also eva_url_new_relative().
 *
...
/* TODO: See RFC 2396 section 5, "Relative URI References"? */
  ...
  EVA_SKIP_WHITESPACE (location);

  if (*location == 0)
    {
      g_set_error (error, EVA_G_ERROR_DOMAIN, EVA_ERROR_INVALID_ARGUMENT,
                   "eva_url_new_relative: location was empty");
      return NULL;
    }
  ...
  if (*tmp == ':')
    {
      /* absolute redirect */
      return eva_url_new (location, error);
    }
  else
    {
      ...
      else
	{
      ...
      if (path && !eva_url_is_valid_path (path, &bad_char))
        {
          g_set_error (error, EVA_G_ERROR_DOMAIN, EVA_ERROR_INVALID_ARGUMENT,
	           "URL %s constructed uses unallowed character '%c' (0x%02x)",
      ...
      if (query && !eva_url_is_valid_query (query, &bad_char))
        {
          g_set_error (error, EVA_G_ERROR_DOMAIN, EVA_ERROR_INVALID_ARGUMENT,
	           "URL %s constructed uses unallowed character '%c' (0x%02x)",
                   "query", bad_char, bad_char);
      ...
      if (fragment && !eva_url_is_valid_fragment (fragment, &bad_char))
        {
          g_set_error (error, EVA_G_ERROR_DOMAIN, EVA_ERROR_INVALID_ARGUMENT,
	           "URL %s constructed uses unallowed character '%c' (0x%02x)",
                   "fragment", bad_char, bad_char);
      ...
      rv = eva_url_new_from_parts (base_url->scheme,
                                   base_url->host,
                                   base_url->port,
                                   base_url->user_name,
                                   base_url->password,
...
#define ADD_CHAR(c)    \
  ...
  if (url->scheme == EVA_URL_SCHEME_FILE)
  ...
  if (url->port == 0)
    {
      switch (url->scheme)
	{
	case EVA_URL_SCHEME_HTTP:
	  return 80;
	case EVA_URL_SCHEME_HTTPS:
	  return 443;
	case EVA_URL_SCHEME_FTP:
	  return 21;

	case EVA_URL_SCHEME_FILE:
	case EVA_URL_SCHEME_OTHER:
	  return 0;
	}
    }
  return url->port;
...
static void
eva_url_get_property (GObject        *object,
		      guint           property_id,
		      GValue         *value,
		      GParamSpec     *pspec)
{
  ...
  switch (property_id)
    ...
    case PROP_PASSWORD:
      ...
      break;
    case PROP_PORT:
      g_value_set_uint (value, eva_url_get_port (url));
      break;
    case PROP_USER_NAME:
...
static void
eva_url_set_property (GObject        *object,
		      guint           property_id,
		      const GValue   *value,
		      GParamSpec     *pspec)
{
...
eva_url_class_init (EvaUrlClass *class)
{
  ...
  object_class->set_property = eva_url_set_property;
  object_class->get_property = eva_url_get_property;
  object_class->finalize = eva_url_finalize;
 ...
 *   g_hash_table_new((GHashFunc)eva_url_hash,
 *                    (GEqualFunc)eva_url_equal);
 *
 ...
 */
char *
eva_url_decode  (const char      *encoded)
{
 ...
 */
char *
eva_url_encode_http (const char *decoded)
{
  const char *at;
  guint len = 0;
  char *rv;
 ...
 */
char *
eva_url_encode_http_binary (const guint8 *decoded,
                            guint         length)
{
  guint rv_len = length;
  char *rv;
 ...
 */
char *
eva_url_decode_http (const char *encoded)
{
  const char *at;
  guint len = 0;
  char *rv;
 ...
 */
char **
eva_url_split_form_urlencoded (const char *encoded_query)
{
...
DONE_SCANNING:
  ...
  for (state = START, rv_at = rv, copy_at = copy; ; ++copy_at)
	...
	case '\0':
	  if (state == GOT_EQUALS)
	    {
	      *rv_at++ = eva_url_decode_http (name);
	      *rv_at++ = eva_url_decode_http (value);
	    }
	  goto DONE;
	case '&':
	  if (state == GOT_EQUALS)
