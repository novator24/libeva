                        ...
                        GskStreamSsl *ssl)
{
  eva_io_set_error (EVA_IO (ssl),
                    ...
                    EVA_IO (transport)->error->code,
                    "SSL: transport layer had error: %s",
                    EVA_IO (transport)->error->message);
}
  ...
  if (want_read && !ssl->backend_poll_read)
    {
      ...
      if (ssl->backend)
	eva_hook_unblock (eva_buffer_stream_read_hook (ssl->backend));
    }
  else if (!want_read && ssl->backend_poll_read)
    {
      ssl->backend_poll_read = FALSE;
      if (ssl->backend)
	eva_hook_block (eva_buffer_stream_read_hook (ssl->backend));
    }
  ...
  if (want_write && !ssl->backend_poll_write)
    {
      ...
      if (ssl->backend)
	eva_hook_unblock (eva_buffer_stream_write_hook (ssl->backend));
    }
  else if (!want_write && ssl->backend_poll_write)
    {
      ssl->backend_poll_write = FALSE;
      if (ssl->backend)
	eva_hook_block (eva_buffer_stream_write_hook (ssl->backend));
    }
}
			      ...
			      GError  **error)
{
  ...
  if (ssl->ssl == NULL)
    {
      eva_io_notify_shutdown (EVA_IO (ssl));
      if (ssl->backend)
        eva_io_shutdown (EVA_IO (ssl->backend), NULL);
      return TRUE;
    }
  ...
  if (rv == 0)
    {
      ...
      ssl->state = EVA_STREAM_SSL_STATE_SHUTTING_DOWN;
      eva_io_write_shutdown (ssl->backend, NULL);
      eva_buffer_stream_read_shutdown (EVA_BUFFER_STREAM (ssl->backend));
      eva_io_notify_shutdown (EVA_IO (ssl));
      return TRUE;
    }
  else if (rv == 1)
    {
      ...
      ssl->state = EVA_STREAM_SSL_STATE_SHUT_DOWN;
			       ...
			       GError    **error)
{
  ...
  if (!eva_io_get_is_writable (io)
    || eva_io_get_is_write_shutting_down (io))
    {
      ...
      return eva_stream_ssl_shutdown_both (ssl, error);
    }
			       ...
			       GError    **error)
{
  ...
  if (!eva_io_get_is_readable (io)
    ...
    || ssl->got_remote_shutdown)
    {
      ...
      eva_stream_ssl_shutdown_both (ssl, error);
      return TRUE;
    }
  ...
  if (ssl_write_rv == 0)
    {
	 ...
	 Either a clean shutdown occurred, or an error occurred in the
	 underlying transport. */
      eva_io_notify_write_shutdown (ssl);
  ...
  if (ssl->backend != NULL)
    {
      ...
      eva_hook_untrap (eva_buffer_stream_write_hook (ssl->backend));
		     ...
		     gpointer      data)
{
  ...
  if (ssl->doing_handshake)
	...
	{
	  eva_io_set_gerror (EVA_IO (ssl), EVA_IO_ERROR_INIT, error);
	  return FALSE;
	}
      return TRUE;
    }
  switch (ssl->state)
    {
    case EVA_STREAM_SSL_STATE_CONSTRUCTING:
      g_return_val_if_reached (FALSE);
    case EVA_STREAM_SSL_STATE_ERROR:
      return FALSE;
    case EVA_STREAM_SSL_STATE_NORMAL:
      if (ssl->backend_poll_write)
	{
	  if (ssl->read_needed_to_write && ssl->this_writable)
	    {
	      ssl->read_needed_to_write = 0;
	      eva_io_notify_ready_to_write (ssl);
	    }
	  else if (ssl->this_readable)
	    eva_io_notify_ready_to_read (ssl);
	}
      return TRUE;
    case EVA_STREAM_SSL_STATE_SHUTTING_DOWN:
      {
	GError *error = NULL;
	eva_stream_ssl_shutdown_both (ssl, &error);
	if (error)
	  eva_io_set_gerror (EVA_IO (ssl), EVA_IO_ERROR_SHUTDOWN_READ, error);
	return TRUE;
      }
    case EVA_STREAM_SSL_STATE_SHUT_DOWN:
      g_return_val_if_reached (FALSE);
    }
  g_return_val_if_reached (FALSE);
}
		     ...
		     gpointer      data)
{
  ...
  switch (ssl->state)
      ...
      if (ssl->doing_handshake)
	{
	  ...
	  if (!do_handshake (ssl, ssl->ssl, &error))
	    {
	      eva_io_set_gerror (EVA_IO (ssl), EVA_IO_ERROR_INIT,
				 error);
	      return FALSE;
	    }
	}
	...
	if (error)
	  eva_io_set_gerror (EVA_IO (ssl), EVA_IO_ERROR_SHUTDOWN_READ, error);
	return ssl->state == EVA_STREAM_SSL_STATE_SHUTTING_DOWN
	    || ssl->state == EVA_STREAM_SSL_STATE_SHUT_DOWN;
...
eva_stream_ssl_init (GskStreamSsl *stream_ssl)
{
  ...
  stream_ssl->ctx = ctx;
  stream_ssl->state = EVA_STREAM_SSL_STATE_CONSTRUCTING;

  eva_io_mark_is_writable (stream_ssl);
  eva_io_mark_is_readable (stream_ssl);
  eva_hook_mark_can_defer_shutdown (EVA_IO_READ_HOOK (stream_ssl));
  eva_hook_mark_can_defer_shutdown (EVA_IO_WRITE_HOOK (stream_ssl));
}
	   ...
	   ...)
{
  ...
  eva_io_set_error (EVA_IO (ssl), EVA_IO_ERROR_INIT,
		    EVA_ERROR_BAD_FORMAT,
		    _("error %s: %s [%s, %d]"),
...
ssl_ctx_setup (GskStreamSsl *ssl)
{
  ...
  if (verify_ca)
	...
	{
	  set_error (ssl, EVA_ERROR_BAD_FORMAT, "loading CA file or directory");
	  return FALSE;
	}
      if (SSL_CTX_set_default_verify_paths (ssl->ctx) != 1)
	{
	  set_error (ssl, EVA_ERROR_BAD_FORMAT, "loading default CA file and/or directory");
	  return FALSE;
	}
    }
  ...
  if (ssl->cert_file != NULL)
    if (SSL_CTX_use_certificate_chain_file (ssl->ctx, ssl->cert_file) != 1)
      {
        set_error (ssl, EVA_ERROR_BAD_FORMAT, "loading certificate from file '%s'",
                   ssl->cert_file);
        return FALSE;
      }
  if (ssl->key_file != NULL)
    if (SSL_CTX_use_PrivateKey_file (ssl->ctx, ssl->key_file, SSL_FILETYPE_PEM) != 1)
      {
        set_error (ssl, EVA_ERROR_BAD_FORMAT, "loading private key from file '%s'",
                   ssl->key_file);
        return FALSE;
      }
...
static GObject*
eva_stream_ssl_constructor    (GType                  type,
  ...
  if (ssl_ctx_setup (ssl))
    {
      ...
      eva_stream_ssl_alloc_backend (ssl);
      ssl->state = EVA_STREAM_SSL_STATE_NORMAL;
    }
  else
    {
      ssl->state = EVA_STREAM_SSL_STATE_ERROR;
    }
  return rv;
}

static int
eva_openssl_passwd_cb (char     *buf,
		       int       size,
...
void
eva_stream_ssl_set_property           (GObject        *object,
				       guint           property_id,
				       const GValue   *value,
				       GParamSpec     *pspec)
{
	...
	if (ssl->password != NULL)
	  {
	    ...
	    SSL_CTX_set_default_passwd_cb (ssl->ctx, eva_openssl_passwd_cb);
	  }
	break;
      case PROP_CERT_FILE:
	arg = g_value_dup_string (value);
...
void
eva_stream_ssl_get_property	      (GObject        *object,
				       guint           property_id,
				       GValue         *value,
				       GParamSpec     *pspec)
{
...
eva_stream_ssl_class_init (GskStreamSslClass *class)
{
  ...
  io_class->set_poll_read = eva_stream_ssl_set_poll_read;
  io_class->shutdown_read = eva_stream_ssl_shutdown_read;
  io_class->set_poll_write = eva_stream_ssl_set_poll_write;
  io_class->shutdown_write = eva_stream_ssl_shutdown_write;
  stream_class->raw_read = eva_stream_ssl_raw_read;
  stream_class->raw_write = eva_stream_ssl_raw_write;
  object_class->constructor = eva_stream_ssl_constructor;
  object_class->get_property = eva_stream_ssl_get_property;
  object_class->set_property = eva_stream_ssl_set_property;
  object_class->finalize = eva_stream_ssl_finalize;
			...
			GError      **error)
{
  if (ssl->state == EVA_STREAM_SSL_STATE_ERROR)
    {
      g_propagate_error (error, EVA_IO (ssl)->error);
      g_object_unref (ssl);
      if (transport != NULL)
  ...
  if (transport != NULL)
    {
      ...
      connection = eva_stream_connection_new (ssl->backend, transport, &suberror);
      if (suberror)
	{
      ...
      eva_stream_attach (transport, ssl->backend, &suberror);
      if (suberror)
	{
	  g_propagate_error (error, suberror);
	  eva_stream_connection_detach (connection);
					  ...
					  GError      **error)
{
  ...
  return EVA_STREAM (stream_ssl);
}

/**
 * eva_stream_ssl_new_client:
 * @cert_file: the PEM x509 certificate file.
 * @key_file: key file???
 * @password: password required by the certificate, or NULL.
 * @transport: optional transport layer (which will be connected
