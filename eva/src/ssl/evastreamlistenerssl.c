#include "evastreamlistenerssl.h"
#include "../evamacros.h"
			  ...
			  GError      **error)
{
  ...
  eva_stream_listener_notify_accepted (EVA_STREAM_LISTENER (listener_ssl), ssl);
  g_object_unref (ssl);
  return TRUE;
}
...
static void
eva_stream_listener_ssl_set_property (GObject         *object,
				      guint            property_id,
				      const GValue    *value,
				      GParamSpec      *pspec)
{
  ...
  switch (property_id)
	...
	{
	  listener_ssl->underlying = eva_hook_untrap (listener_ssl);
	  g_assert (listener_ssl->underlying == NULL);
	}
#endif
      listener_ssl->underlying = EVA_STREAM_LISTENER (g_value_dup_object (value));
      eva_stream_listener_handle_accept (listener_ssl->underlying,
...
static void
eva_stream_listener_ssl_get_property (GObject         *object,
				      guint            property_id,
				      GValue          *value,
				      GParamSpec      *pspec)
{
...
eva_stream_listener_ssl_class_init (GskStreamListenerSslClass *class)
{
  ...
  object_class->finalize = eva_stream_listener_ssl_finalize;
  object_class->set_property = eva_stream_listener_ssl_set_property;
  object_class->get_property = eva_stream_listener_ssl_get_property;

  pspec = g_param_spec_object ("underlying",
			       _("Underlying Listener"),
			       _("the unencrypted underlying transport"),
			       EVA_TYPE_STREAM_LISTENER,
...
GType eva_stream_listener_ssl_get_type()
{
			     ...
			     const char        *key_file)
{
  ...
  return g_object_new (EVA_TYPE_STREAM_LISTENER_SSL,
