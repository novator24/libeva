...
parsed_format_new (const char *output_format)
{
  ...
  if (output_format == NULL)
    output_format = EVA_LOG_DEFAULT_OUTPUT_FORMAT;
...
handle_ring_buffer (const char *domain,
                    ...
                    gpointer    data)
{
  eva_log_ring_buffer_add (data, formatted_message);
}
                                       ...
                                       GLogLevelFlags trap_mask)
{
  if (!log_system_initialized)
    eva_log_init ();
  return eva_log_trap_generic (domain, trap_mask, "",
...
/* --- support for eva_log_rotate_stdio_logs() --- */
...
{
  guint time = eva_main_loop_default ()->current_time.tv_sec;
...
void
eva_log_rotate_stdio_logs (const char *output_file_template,
