...
#define USE_GLIB_MAIN_LOOP	EVA_PACKET_QUEUE_FD_USE_GLIB_MAIN_LOOP
		          ...
		          GError           **error)
{
  ...
  if (!eva_socket_address_to_native (addr, native, error))
    return FALSE;
  ...
  eva_packet_queue_mark_allow_no_address (queue);
  return TRUE;
}
		           ...
		           GError           **error)
{
  ...
  if (out->dst_address != NULL)
    {
      native_size = eva_socket_address_sizeof_native (out->dst_address);
      native_addr = alloca (native_size);
      if (!eva_socket_address_to_native (out->dst_address, native_addr, error))
	return FALSE;
    }
  else
    {
...
/* The following functions are defined twice:
      ...
      set_poll_read()
      set_poll_write()
   once for glib and once for eva.
 */
...
static gboolean
eva_packet_queue_fd_source_prepare (GSource    *source,
			            gint       *timeout)
{
  return FALSE;
}
...
static gboolean
eva_packet_queue_fd_source_dispatch (GSource    *source,
			             ...
			             gpointer    user_data)
{
  ...
  if ((events & G_IO_OUT) == G_IO_OUT)
    eva_io_notify_ready_to_write (EVA_IO (packet_queue_fd));
  return TRUE;
}

static GSourceFuncs eva_packet_queue_fd_source_funcs =
{
  eva_packet_queue_fd_source_prepare,
  eva_packet_queue_fd_source_check,
  eva_packet_queue_fd_source_dispatch,
  NULL,					/* finalize */
...
remove_poll (GskPacketQueueFd *packet_queue_fd)
{
  if (packet_queue_fd->source != NULL)
    {
      eva_source_remove (packet_queue_fd->source);
      packet_queue_fd->source = NULL;
    }
}
static void
			             ...
			             gboolean       do_poll)
{
  ...
  if (do_poll)
    ...
    eva_source_remove_io_events (packet_queue_fd->source, G_IO_OUT);
}
#endif
  ...
  if (packet_queue_fd->fd >= 0)
    {
      ...
      eva_fork_remove_cleanup_fd (packet_queue_fd->fd);
...
/* --- arguments --- */
static void
eva_packet_queue_fd_get_property (GObject        *object,
			          guint           property_id,
			          GValue         *value,
			          GParamSpec     *pspec)
{
  switch (property_id)
    ...
    case PROP_FILE_DESCRIPTOR:
      g_value_set_int (value, EVA_PACKET_QUEUE_FD (object)->fd);
      break;
    }
}

static void
eva_packet_queue_fd_set_property (GObject        *object,
			          guint           property_id,
			          const GValue   *value,
			          GParamSpec     *pspec)
{
		          ...
		          GError   **error)
{
  ...
  EVA_HOOK_SET_FLAG (EVA_IO_WRITE_HOOK (queue), IS_AVAILABLE);
  EVA_HOOK_SET_FLAG (EVA_IO_READ_HOOK (queue), IS_AVAILABLE);
  return TRUE;
}
...
eva_packet_queue_fd_class_init (GskPacketQueueFdClass *class)
{
  ...
  queue_class->bind = eva_packet_queue_fd_bind;
  queue_class->read = eva_packet_queue_fd_read;
  queue_class->write = eva_packet_queue_fd_write;
  io_class->set_poll_read = eva_packet_queue_fd_set_poll_read;
  io_class->set_poll_write = eva_packet_queue_fd_set_poll_write;
  io_class->shutdown_read = eva_packet_queue_fd_shutdown_read;
  io_class->shutdown_write = eva_packet_queue_fd_shutdown_write;
  io_class->open = eva_packet_queue_fd_open;
  io_class->close = eva_packet_queue_fd_close;
  object_class->get_property = eva_packet_queue_fd_get_property;
  object_class->set_property = eva_packet_queue_fd_set_property;
...
GType eva_packet_queue_fd_get_type()
{
...
eva_packet_queue_fd_new           (int  fd)
{
  ...
  return g_object_new (EVA_TYPE_PACKET_QUEUE_FD, "file-descriptor", fd, NULL);
}

/**
 * eva_packet_queue_fd_new_by_family:
...
retry:
  ...
  return eva_packet_queue_fd_new (fd);
}

/**
 * eva_packet_queue_fd_new_bound:
...
/**
 * eva_packet_queue_fd_set_broadcast:
					    ...
					    GError          **error)
{
  ...
  if (setsockopt (fd, SOL_SOCKET, SO_BROADCAST, &broadcast, sizeof (int)) < 0)
    {
      g_set_error (error, EVA_G_ERROR_DOMAIN, eva_error_code_from_errno (errno),
