...
#include "evautils.h"
#include "evamemorybarrier.h"
#include "evatable.h"
#include "evatable-file.h"
#include "evaerror.h"
...
file_info_unref (FileInfo *fi, const char *dir, gboolean erase)
{
  ...
  if (--(fi->ref_count) == 0)
    {
      GError *error = NULL;
      if (!eva_table_file_destroy (fi->file, dir, erase, &error))
        {
          g_warning ("eva_table_file_destroy "ID_FMT" (erase=%u) failed: %s",
                           ...
                           MergeTask *to_kill)
{
  ...
  EVA_RBTREE_REMOVE (GET_UNSTARTED_MERGE_TASK_TREE (table), to_kill);
                             ...
                             FileInfo *next)
{
  ...
  EVA_RBTREE_INSERT (GET_UNSTARTED_MERGE_TASK_TREE (table),
                     task, unused);
              ...
              GError   **error)
{
  ...
  if (fstat (fd, &stat_buf) < 0)
    {
      g_set_error (error, EVA_G_ERROR_DOMAIN, EVA_ERROR_FILE_STAT,
  ...
  if (mmapped_journal == MAP_FAILED || mmapped_journal == NULL)
    {
      g_set_error (error, EVA_G_ERROR_DOMAIN, EVA_ERROR_FILE_MMAP,
  ...
  if (magic != JOURNAL_FILE_MAGIC)
    {
      g_set_error (error, EVA_G_ERROR_DOMAIN, EVA_ERROR_PARSE,
  ...
  for (i = 0; i < n_files; i++)
    {
      ...
      file_info->file
        = eva_table_file_factory_open_file (file_factory, table->dir,
                                            file_id, error);
  ...
  for (i = 0; i < n_merge_tasks; i++)
    {
      ...
      for (j = 0; j < 2; j++)
        {
          ...
          if (reader == NULL)
            {
              if (j == 1)
                eva_table_reader_destroy (merge_task->info.started.inputs[0].reader);
      ...
      merge_task->info.started.output
        = eva_table_file_factory_open_building_file (file_factory,
                                                     table->dir, output_file_id,
                                                     build_state_len,
                                                     build_state, error);
      if (merge_task->info.started.output == NULL)
        {
          eva_g_error_add_prefix (error,
                                "instantiating merge-task between files "ID_FMT" and "ID_FMT,
                                inputs[0].file_id,
                                inputs[1].file_id);
          eva_table_reader_destroy (merge_task->info.started.inputs[0].reader);
          eva_table_reader_destroy (merge_task->info.started.inputs[1].reader);
          g_slice_free (MergeTask, merge_task);
          goto error_cleanup;
        }
      ...
      EVA_STACK_PUSH (GET_RUN_STACK (table), merge_task);
...
#define COMPARE_RUNNING_TASKS_BY_N_INPUTS(a,b, rv)              \
  ...
  EVA_STACK_SORT (GET_RUN_STACK (table), COMPARE_RUNNING_TASKS_BY_N_INPUTS);
#undef COMPARE_RUNNING_TASKS_BY_N_INPUTS
  ...
  table->journal_mode = EVA_TABLE_JOURNAL_NONE;
  for (;;)
    {
...
#endif
      at += 8;
      if (!eva_table_add (table, key_len, at, value_len, at + key_len, error))
        {
          eva_g_error_add_prefix (error, "error replaying journal");
          table->journal_mode = old_journal_mode;
          /* do not use error_cleanup, let eva_table_destroy() do the work */
          return FALSE;
        }
                ...
                GError   **error)
{
  ...
  if (ftruncate (journal_fd, new_size) < 0)
    {
      g_set_error (error, EVA_G_ERROR_DOMAIN, EVA_ERROR_FILE_TRUNCATE,
               ...
               GError    **error)
{
    ...
    if (fi->next_task != NULL && fi->next_task->is_started)
      {
        ...
        for (input = 0; input < 2; input++)
          {
            if (!eva_table_file_get_reader_state (task->inputs[input]->file,
                                                  task->info.started.inputs[input].reader,
                                                  &reader_state_lens[input],
                                                  &reader_states[input],
                                                  error))
              {
                eva_g_error_add_prefix (error, "reset_journal: input %u", input);
                goto failed_writing_journal;
              }
          }
        if (!eva_table_file_get_build_state (task->info.started.output,
                                             &build_state_len,
                                             &build_state,
                                             error))
          {
            eva_g_error_add_prefix (error, "reset_journal: build state");
            goto failed_writing_journal;
          }
  ...
  if (rename (table->journal_tmp_fname, table->journal_cur_fname) < 0)
    {
      g_set_error (error, EVA_G_ERROR_DOMAIN, EVA_ERROR_FILE_RENAME,
...
#define TMP_KEY_COMPARATOR(unused, at, rv) \
  ...
  EVA_RBTREE_LOOKUP_COMPARATOR (GET_IN_MEMORY_TREE (table),
...
#if 0
  {
    char *a_hex = eva_escape_memory_hex (a, a_len);
    char *b_hex = eva_escape_memory_hex (b, b_len);
 ...
 * equivalent to EVA_TABLE_MAY_CREATE|EVA_TABLE_MAY_EXIST.
 *
                       ...
                       GError               **error)
{
  ...
  if (g_file_test (dir, G_FILE_TEST_IS_DIR))
    {
      if ((new_flags & EVA_TABLE_MAY_EXIST) == 0)
        {
          g_set_error (error, EVA_G_ERROR_DOMAIN, EVA_ERROR_EXISTS,
    ...
    {
      if ((new_flags & EVA_TABLE_MAY_CREATE) == 0)
        {
          g_set_error (error, EVA_G_ERROR_DOMAIN, EVA_ERROR_EXISTS,
                       "table dir %s already exists", dir);
          return NULL;
      ...
      did_mkdir = TRUE;
      if (!eva_mkdir_p (dir, 0755, error))
        return FALSE;
    }

  lock_fd = eva_lock_dir (dir, FALSE, error);
  if (lock_fd < 0)
    return FALSE;
  ...
  if (did_mkdir)
    {
      ...
      if (!reset_journal (table, error))
        {
          eva_table_destroy (table);
          eva_rm_rf (table->dir, NULL);
          return NULL;
        }
    }
  else
      ...
      if (!read_journal (table, error))
        {
          GError *e = NULL;
          g_free (table->dir);
          if (!eva_unlock_dir (table->lock_fd, &e))
            {
                  ...
                  GError    **error)
{
  ...
  EVA_RBTREE_REMOVE (GET_UNSTARTED_MERGE_TASK_TREE (table), merge_task);

  for (input = 0; input < 2; input++)
    {
      readers[input] = eva_table_file_create_reader (merge_task->inputs[input]->file,
                                                     table->dir,
                                                     error);
      if (readers[input] == NULL)
        {
          eva_g_error_add_prefix (error, "creating merge job: error making reader %u", input);
          if (input == 1)
            eva_table_reader_destroy (readers[0]);
          return FALSE;
        }
    }

  output_file_id = ++(table->last_file_id);
  output = eva_table_file_factory_create_file (table->file_factory,
                                               table->dir,
                                               output_file_id,
                                               &file_hints,
                                               error);
  if (output == NULL)
    {
      eva_g_error_add_prefix (error, "creating merge-task output");
      eva_table_reader_destroy (readers[0]);
      eva_table_reader_destroy (readers[1]);
      return FALSE;
    }
  ...
  eva_table_buffer_init (&merge_task->info.started.last_queryable_key);
                   ...
                   GError  **error)
{
  while (table->n_running_tasks < table->max_running_tasks
      && table->unstarted_merges != NULL)
    {
      ...
      EVA_RBTREE_FIRST (GET_UNSTARTED_MERGE_TASK_TREE (table), bottom_heaviest);
                       ...
                       GError     **error)
{
  if (node->left != NULL
   && !dump_tree_recursively (node->left, file, error))
    return FALSE;
  if (eva_table_file_feed_entry (file, node->key.len, node->key.data,
                                 node->value.len, node->value.data,
                                 error) == EVA_TABLE_FEED_ENTRY_ERROR)
    return FALSE;
  if (node->right != NULL
            ...
            GError    **error)
{
  ...
  if (file == NULL)
    {
      eva_g_error_add_prefix (error, "flushing in-memory tree");
      return FALSE;
    }
  if (!dump_tree_recursively (table->in_memory_tree, file, error))
    {
      eva_g_error_add_prefix (error, "dumping in-memory tree");
      eva_table_file_destroy (file, table->dir, TRUE, NULL);
      return FALSE;
    }
  if (!eva_table_file_done_feeding (file, &done, error))
    {
      eva_g_error_add_prefix (error, "finishing flushing in-memory tree");
      eva_table_file_destroy (file, table->dir, TRUE, NULL);
      return FALSE;
    }
  if (done == FALSE)
  ...
  table->n_files++;
  EVA_LIST_APPEND (GET_FILE_INFO_LIST (table), fi);
...
{
  TreeNode *found;
  gboolean must_write_journal = table->journal_mode == EVA_TABLE_JOURNAL_DEFAULT;
  ...
  if (found)
    {
      ...
      switch (merge_result)
        {
        case EVA_TABLE_MERGE_RETURN_A:
          /* nothing to do */
          break;
        case EVA_TABLE_MERGE_RETURN_B:
        ...
        case EVA_TABLE_MERGE_SUCCESS:
          ...
          break;
        case EVA_TABLE_MERGE_DROP:
          EVA_RBTREE_REMOVE (GET_IN_MEMORY_TREE (table), found);
  ...
  else
    {
      ...
      /* TODO: this repeats the work of the lookup.
         maybe we need a EVA_RBTREE_INSERT_NO_REPLACE()
         which can be used at the outset instead of LOOKUP_COMPARATOR? */
      EVA_RBTREE_INSERT (GET_IN_MEMORY_TREE (table), new, found);
      g_assert (found == NULL);
   ...
   || table->in_memory_bytes >= table->max_in_memory_bytes)
    {
      ...
      if (!flush_tree (table, error))
        {
          eva_g_error_add_prefix (error, "flushing tree");
          return FALSE;
        }

      /* maybe flush journal */
      if (table->journal_mode != EVA_TABLE_JOURNAL_NONE
       && (++(table->journal_flush_index) == table->journal_flush_period))
        {
          /* write new journal */
          if (!reset_journal (table, error))
            {
              eva_g_error_add_prefix (error, "error flushing journal");
              return FALSE;
            }
      ...
      if (new_journal_len + 4 > table->journal_size)
        {
          if (!resize_journal (table->journal_fd,
                               ...
                               error))
            {
              eva_g_error_add_prefix (error, "expanding journal");
              return FALSE;
            }
        }
#if DEBUG_JOURNAL_WRITING
                       ...
                       GError               **error)
{
  ...
  {
    char *hex = eva_escape_memory_hex (key_data, key_len);
          ...
          if (mt->info.started.has_last_queryable_key)
            {
              ...
              if (!eva_table_file_query (mt->info.started.output,
                                         query, error))
                {
                  eva_g_error_add_prefix (error, "querying merge-task output");
                  goto failed;
                }
...
cannot_use_merge_output:
      ...
      /* query file */
      if (!eva_table_file_query (fi->file, query, error))
        {
          eva_g_error_add_prefix (error, "querying merge-task output");
          goto failed;
        }
...
eva_table_destroy     (GskTable              *table)
{
  ...
  munmap (table->journal_mmap, table->journal_size);
  eva_table_buffer_clear (&table->result_buffers[0]);
                 ...
                 GError     **error)
{
  ...
  if (!eva_table_file_done_feeding (task->info.started.output, &done, error))
    return FALSE;
  if (done == FALSE)
    g_error ("eva_table_file_done_feeding not ready not handled yet");

  /* destroy the input readers */
  eva_table_reader_destroy (task->info.started.inputs[0].reader);
  eva_table_reader_destroy (task->info.started.inputs[1].reader);
  ...
  CHECK_FILES_CONTIGUOUS (table);
  EVA_LIST_INSERT_BEFORE (GET_FILE_INFO_LIST (table), task->inputs[0], new_file);
  EVA_LIST_REMOVE (GET_FILE_INFO_LIST (table), task->inputs[0]);
  EVA_LIST_REMOVE (GET_FILE_INFO_LIST (table), task->inputs[1]);
  table->n_files -= 1;
  CHECK_FILES_CONTIGUOUS (table);
...
#include "evatable-implementations-generated.c"
...
#define TEST_MEMBER_NULL(member) \
  if (options->member != NULL) \
    { \
      g_set_error (error, EVA_G_ERROR_DOMAIN, EVA_ERROR_INVALID_ARGUMENT, \
