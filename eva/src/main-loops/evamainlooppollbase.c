...
{
  char buf[4096];
  g_return_val_if_fail (EVA_IS_MAIN_LOOP_POLL_BASE (user_data), FALSE);
  if ((condition & G_IO_IN) != G_IO_IN)
    return TRUE;
...
eva_main_loop_poll_base_init_wakeup (EvaMainLoopPollBase *poll_base)
{
  ...
  poll_base->wakeup_read_pipe = eva_main_loop_add_io (EVA_MAIN_LOOP (poll_base),
		                    ...
		                    EvaMainLoopChange     *change)
{
  ...
  switch (change->type)
	...
	if (poll_base->wakeup_write_fd == -1)
	  eva_main_loop_poll_base_init_wakeup (poll_base);
		 ...
		 || g_array_index (signal_fds, int, signal_fd_index) == -1)
		  {
		    ...
		    eva_fd_set_nonblocking (pipe_fds[0]);
		    eva_fd_set_nonblocking (pipe_fds[1]);
		    signal_source = eva_main_loop_add_io (main_loop, pipe_fds[0], G_IO_IN,
	...
	if (change->data.process.add)
	  {
	    ...
	    if (waitpid_dispatcher == NULL)
	      {
		waitpid_dispatcher = eva_main_loop_add_signal (main_loop,
							       SIGCHLD,
							       eva_main_loop_handle_sigchld,
							       NULL,
							       NULL);
                             ...
                             gint                timeout)
{
  ...
  if (poll_base->try_waitpid)
	...
	{
	  events[n_init].type = EVA_MAIN_LOOP_EVENT_PROCESS;
	...
	timeout = 0;
#else
       eva_main_loop_handle_sigchld (SIGCHLD, NULL);
#endif
  ...
  while (tmp < max_events_out)
	...
	break;
      ...
      events[tmp].type = EVA_MAIN_LOOP_EVENT_PROCESS;
  ...
  while (tmp < max_events_out)
    {
      ...
      int sig;
      G_LOCK (waitpid_dispatcher);
      n_read = eva_buffer_read (&poll_base->signal_ids, &sig, sizeof (sig));
      G_UNLOCK (waitpid_dispatcher);
      if (n_read == 0)
	break;
      g_assert (n_read == sizeof (sig));
      events[tmp].type = EVA_MAIN_LOOP_EVENT_SIGNAL;
...
static void eva_main_loop_poll_base_init (EvaMainLoopPollBase* poll_base)
{
  ...
  eva_buffer_construct (&poll_base->process_term_notifications);
...
eva_main_loop_poll_base_class_init (EvaMainLoopClass* loop_class)
{
  ...
  loop_class->change = eva_main_loop_poll_base_change;
  loop_class->poll = eva_main_loop_poll_base_poll;
  object_class->finalize = eva_main_loop_poll_base_finalize;
}


GType eva_main_loop_poll_base_get_type()
{
