  ...
  if (block && !stream_connection->blocking_write_side)
    {
      ...
      eva_io_block_write (EVA_IO (stream_connection->write_side));
    }
  else if (!block && stream_connection->blocking_write_side)
    {
      stream_connection->blocking_write_side = 0;
      eva_io_unblock_write (EVA_IO (stream_connection->write_side));
    }
}
  ...
  if (block && !stream_connection->blocking_read_side)
    {
      ...
      eva_io_block_read (EVA_IO (stream_connection->read_side));
    }
  else if (!block && stream_connection->blocking_read_side)
    {
      stream_connection->blocking_read_side = 0;
      eva_io_unblock_read (EVA_IO (stream_connection->read_side));
    }
}
static inline void
...
/**
 * eva_stream_connection_set_max_buffered:
...
/**
 * eva_stream_connection_get_max_buffered:
 ...
 *
 * The actual number of bytes of data can be found with eva_stream_connection_get_cur_buffered().
 *
...
/**
 * eva_stream_connection_get_cur_buffered:
 * @connection: the connection to query.
 *
...
/**
 * eva_stream_connection_set_atomic_read_size:
 * @connection: the connection to affect.
...
/**
 * eva_stream_connection_get_atomic_read_size:
 * @connection: the connection to query.
 *
 * Set the number of bytes to read atomically from
			  ...
			  gpointer       data)
{
  ...
  if (stream_connection->use_read_buffer)
    num_read = eva_stream_read_buffer (read_side, &stream_connection->buffer, &error);
  else
  ...
  if (buf != NULL)
    {
      ...
      if (num_written < num_read)
        eva_buffer_append (&stream_connection->buffer,
                           buf + num_written,
                           num_read - num_written);
    }
  else if (stream_connection->buffer.size > 1024)
    {
      eva_stream_write_buffer (write_side,
  ...
  if (stream_connection->write_side != NULL)
    {
      ...
      if (stream_connection->buffer.size == 0)
	{
	  if (!eva_io_write_shutdown (EVA_IO (stream_connection->write_side), &error)
	      && error != NULL)
	    {
	      const char *msg = error->message;
	      eva_g_debug ("stream-attach: handle-read-shutdown: doing write-shutdown: %s", msg);
	      if (error)
		g_error_free (error);
	    }
	}
			   ...
			   gpointer       data)
{
  ...
  if (stream_connection->buffer.size > 0)
    {
      eva_stream_write_buffer (write_side, &stream_connection->buffer, &error);
      if (error)
	{
	  handle_error (stream_connection, error);
	  return TRUE;
  ...
  if (stream_connection->buffer.size == 0
   && read_side == NULL)
	...
	{
	  const char *msg = error->message;
	  eva_g_debug ("stream-attach: handle-output-is-writable, shutting down write end: %s", msg);
	  if (error)
	    g_error_free (error);
	}
    }
			      ...
			      gpointer   data)
{
  ...
  if (stream_connection->read_side != NULL)
    {
      GError *error = NULL;
      if (!eva_io_read_shutdown (EVA_IO (stream_connection->read_side), &error)
	  && error != NULL)
        {
	  const char *msg = error->message;
...
handle_output_is_writable_destroy (gpointer data)
{
  ...
  if (stream_connection->read_side != NULL)
    eva_io_untrap_readable (EVA_IO (stream_connection->read_side));
...
eva_stream_connection_init (GskStreamConnection *stream_connection)
{
  ...
  eva_buffer_construct (&stream_connection->buffer);
}
...
GType eva_stream_connection_get_type()
{
...
eva_stream_connection_detach (GskStreamConnection *connection)
{
  ...
  if (connection->read_side)
    eva_stream_untrap_readable (connection->read_side);
  
  if (connection->write_side)
    eva_stream_untrap_writable (connection->write_side);
...
/**
 * eva_stream_connection_shutdown:
