 ...
 */ 
#define EVA_DEBUG_BUFFER_ALLOCATIONS	0
...
/**
 * eva_buffer_cleanup_recycling_bin:
 * 
		  ...
		  guint         length)
{
  ...
  while (length > 0)
      ...
      if (!buffer->last_frag)
	{
	  ...
	  avail = eva_buffer_fragment_avail (buffer->last_frag);
	}
      else
	{
	  avail = eva_buffer_fragment_avail (buffer->last_frag);
      ...
      if (avail > length)
	avail = length;
      memcpy (eva_buffer_fragment_end (buffer->last_frag), data, avail);
                                 ...
                                 gsize         count)
{
  ...
  while (count > 0)
      ...
      else
	{
      ...
      if (avail > count)
	avail = count;
      memset (eva_buffer_fragment_end (buffer->last_frag), character, avail);
      count -= avail;
      buffer->last_frag->buf_length += avail;
    }
  CHECK_INTEGRITY (buffer);
                         ...
                         const char *string)
{
  ...
  eva_buffer_append (buffer, string, strlen (string));
}

/**
 * eva_buffer_append_char:
		...
		guint         max_length)
{
  ...
  while (max_length > 0 && buffer->first_frag)
	...
	{
	  memcpy (data, eva_buffer_fragment_start (first), max_length);
	  rv += max_length;
 ...
 * is returned.
 *
 * This function is just like eva_buffer_read() except that the 
		     ...
		     guint            max_length)
{
  ...
  while (max_length > 0 && frag)
	...
	{
	  memcpy (data, eva_buffer_fragment_start (frag), frag->buf_length);
	  rv += frag->buf_length;
	...
	{
	  memcpy (data, eva_buffer_fragment_start (frag), max_length);
	  rv += max_length;
	  data = (char *) data + max_length;
	  max_length = 0;
	}
...
eva_buffer_read_line(EvaBuffer *buffer)
{
  ...
  for (at = buffer->first_frag; at; at = at->next)
    {
      char *start = eva_buffer_fragment_start (at);
      char *got;
      got = memchr (start, '\n', at->buf_length);
      if (got)
	{
  ...
  if (at)
    ...
    newline_length = 0;
  eva_buffer_read (buffer, rv, len + newline_length);
  rv[len] = 0;
  CHECK_INTEGRITY (buffer);
  return rv;
}
...
eva_buffer_parse_string0(EvaBuffer *buffer)
{
  ...
  eva_buffer_read (buffer, rv, index0 + 1);
  return rv;
}

/**
 * eva_buffer_peek_char:
  ...
  for (i = 0; i < nfrag; i++)
    {
      ...
      iov[i].iov_base = eva_buffer_fragment_start (frag_at);
      frag_at = frag_at->next;
    }
  rv = writev (fd, iov, nfrag);
  if (rv < 0 && eva_errno_is_ignorable (errno))
    return 0;
  if (rv <= 0)
    return rv;
  eva_buffer_discard (read_from, rv);
  return rv;
}

/**
 * eva_buffer_writev_len:
 * @read_from: buffer to take data from.
 * @fd: file-descriptor to write data to.
                      ...
                      int        read_from)
{
  ...
  if (rv < 0)
    return rv;
  eva_buffer_append (write_to, buf, rv);
  return rv;
}
		  ...
		  EvaBuffer *src)
{
  ...
  for (frag = src->first_frag; frag; frag = frag->next)
    eva_buffer_append (dst,
                       eva_buffer_fragment_start (frag),
                       frag->buf_length);
  eva_buffer_discard (src, src->size);
  CHECK_INTEGRITY (dst);
  CHECK_INTEGRITY (src);
  return rv;
}
  ...
  for (frag = src->first_frag; frag && max_transfer > 0; frag = frag->next)
    {
      ...
      if (len >= max_transfer)
        ...
        {
          eva_buffer_append (dst, eva_buffer_fragment_start (frag), len);
          rv += len;
          max_transfer -= len;
        }
    }
  eva_buffer_discard (src, rv);
  CHECK_INTEGRITY (dst);
  CHECK_INTEGRITY (src);
  return rv;
}
					 ...
					 ...)
{
  ...
  eva_buffer_vprintf (buffer, format, args);
  va_end (args);
}
  ...
  if (size < 1024)
    {
      ...
      eva_buffer_append_string (buffer, buf);
    }
  else
    {
      char *buf = g_strdup_vprintf (format, args);
      eva_buffer_append_foreign (buffer, buf, strlen (buf), g_free, buf);
    }
}

/* --- eva_buffer_polystr_index_of implementation --- */
  ...
  for (;;)
    {
      ...
      if (memcmp (string,
                  eva_buffer_fragment_start (frag) + frag_index,
                  test_len) != 0)
        return FALSE;
  ...
  for (frag = buffer->first_frag; frag != NULL; frag = frag->next)
    {
      ...
      frag_start = eva_buffer_fragment_start (frag);
      at = frag_start;
      while (at != NULL)
        {
          const char *start = at;
			       ...
			       EvaBuffer         *to_iterate)
{
  ...
  if (iterator->fragment != NULL)
    {
      iterator->in_cur = 0;
      iterator->cur_data = (guint8*)eva_buffer_fragment_start (iterator->fragment);
			       ...
			       guint              max_length)
{
  ...
  while (fragment != NULL)
      ...
      if (out_remaining <= frag_remaining)
	{
      ...
      if (fragment != NULL)
	{
	  frag_data = (guint8 *) eva_buffer_fragment_start (fragment);
