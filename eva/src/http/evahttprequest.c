                                 ...
                                 GParamSpec     *pspec)
{
  ...
  switch (property_id)
    ...
    case PROP_REQUEST_USER_AGENT:
      eva_http_header_set_string_val (request, &request->user_agent, value);
      break;
    case PROP_REQUEST_PATH:
      eva_http_header_set_string_val (request, &request->path, value);
      break;
    case PROP_REQUEST_REFERRER:
      eva_http_header_set_string_val (request, &request->referrer, value);
      break;
    case PROP_REQUEST_HOST:
      eva_http_header_set_string_val (request, &request->host, value);
      break;
    case PROP_REQUEST_MAX_FORWARDS:
      request->max_forwards = g_value_get_int (value);
      break;
    case PROP_REQUEST_FROM:
      eva_http_header_set_string_val (request, &request->from, value);
      break;
    default:
...
static void
eva_http_request_get_property   (GObject        *object,
                                 guint           property_id,
                                 GValue         *value,
                                 GParamSpec     *pspec)
{
...
#undef FREE_LIST
  ...
  eva_http_header_free_string (request, request->user_agent);
  eva_http_header_free_string (request, request->referrer);
  eva_http_header_free_string (request, request->from);
  if (request->authorization)
    eva_http_authorization_unref (request->authorization);
  if (request->proxy_authorization)
    eva_http_authorization_unref (request->proxy_authorization);
  if (NULL != request->cache_control)
    {
      eva_http_request_cache_directive_free (request->cache_control);
    }
  g_free (request->ua_color);
  g_free (request->ua_os);
  g_free (request->ua_cpu);
  g_free (request->ua_language);
  g_slist_foreach (request->cookies, (GFunc) eva_http_cookie_free, NULL);
...
static void
eva_http_request_class_init (GObjectClass *class)
{
  parent_class = g_type_class_peek_parent (class);
  class->finalize = eva_http_request_finalize;
  class->get_property = eva_http_request_get_property;
  class->set_property = eva_http_request_set_property;
...
GType eva_http_request_get_type()
{
...
/**
 * eva_http_request_has_content_body:
...
eva_http_request_has_content_body (GskHttpRequest *request)
{
  switch (request->verb)
    {
    case EVA_HTTP_VERB_GET:
    case EVA_HTTP_VERB_HEAD:
    case EVA_HTTP_VERB_OPTIONS:
    case EVA_HTTP_VERB_DELETE:
    case EVA_HTTP_VERB_CONNECT:	//???
    case EVA_HTTP_VERB_TRACE:	//???
      return FALSE;
    case EVA_HTTP_VERB_POST:
    case EVA_HTTP_VERB_PUT:
      return TRUE;
      break;
    }
 ...
 * it SHOULD response with an error (#EVA_HTTP_STATUS_NOT_ACCEPTABLE)
...
/**
 * eva_http_request_clear_charsets:
 * @header: the request to affect.
 *
 ...
 *
 * Note that the EVA http server and client handle content
...
/**
 * eva_http_request_clear_content_encodings:
 * @header: the request to affect.
 *
...
/**
 * eva_http_request_clear_transfer_encodings:
 * @header: the request to affect.
 *
 ...
 * with quality factors as for eva_http_request_add_charsets().
 *
...
/**
 * eva_http_request_set_authorization:
					  ...
					  GskHttpAuthorization *auth)
{
  ...
  if (*dst_auth)
    eva_http_authorization_unref (*dst_auth);
  *dst_auth = auth;
}

/**
 * eva_http_request_peek_authorization:
 * @request: the request to query.
...
/**
 * eva_http_request_remove_cookie:
 * @header: the request to affect.
				  ...
				  GskHttpCookie  *cookie)
{
  ...
  eva_http_cookie_free (cookie);
}

/**
 * eva_http_request_find_cookie:
...
GHashTable *
eva_http_request_parse_cgi_query_string  (const char     *query_string)
{
  char **cgi_vars = eva_http_parse_cgi_query_string (query_string, NULL);
  GHashTable *table;
  guint i;
  if (cgi_vars == NULL)
    return NULL;
...
/**
 * eva_http_parse_cgi_query_string:
 ...
 */
char **
eva_http_parse_cgi_query_string  (const char     *query_string,
                                  GError        **error)
{
