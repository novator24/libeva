                                    ...
                                    EvaHttpRequest  *request)
{
  ...
  switch (response->status_code)
	...
	case EVA_HTTP_STATUS_MULTIPLE_CHOICES:
          if (request->verb == EVA_HTTP_VERB_PUT
           || request->verb == EVA_HTTP_VERB_DELETE) /* HEAD handled above */
            return FALSE;
          else
            return TRUE;
	  switch (request->verb)
  ...
  g_warning ("eva_http_response_has_content_body: unknown status code %u",
             response->status_code);
  return FALSE;
}

static void
eva_http_response_set_property  (GObject        *object,
                                 guint           property_id,
                                 const GValue   *value,
                                 GParamSpec     *pspec)
{
...
static void
eva_http_response_get_property  (GObject        *object,
                                 guint           property_id,
                                 GValue         *value,
                                 GParamSpec     *pspec)
{
...
eva_http_response_finalize (GObject *object)
{
  ...
  if (response->set_cookies)
    {
      g_slist_foreach (response->set_cookies, (GFunc) eva_http_cookie_free, NULL);
...
static void
eva_http_response_class_init (GObjectClass *class)
{
  GObjectClass *object_class = G_OBJECT_CLASS (class);
  parent_class = g_type_class_peek_parent (class);
  class->finalize = eva_http_response_finalize;
  class->get_property = eva_http_response_get_property;
  class->set_property = eva_http_response_set_property;
...
GType eva_http_response_get_type()
{
...
/* --- public methods --- */
/**
 * eva_http_response_set_cache_control:
 ...
 * EVA does not attempt to verify the MD5 sum;
...
/**
 * eva_http_response_set_retry_after:
...
/**
 * eva_http_response_set_no_retry_after:
...
/**
 * eva_http_response_set_authenticate:
					  ...
					  EvaHttpAuthenticate *auth)
{
  ...
  if (auth)
    eva_http_authenticate_ref (auth);
  if (*dst_auth != NULL)
    eva_http_authenticate_unref (*dst_auth);

  *dst_auth = auth;
}

/**
 * eva_http_response_peek_authenticate:
 * @response: the response to query.
...
/**
 * eva_http_response_set_allowed_verbs:
 * @response: the response to affect.
 * @allowed: bits telling which type of requests this server allows.
 * The bit for a verb 'Q' is (1 &lt;&lt; EVA_HTTP_VERB_Q);
 * the default allowed bits are (1&lt;&lt;EVA_HTTP_VERB_GET) | (1&lt;&lt;EVA_HTTP_VERB_HEAD).
 *
...
/**
 * eva_http_response_new_redirect:
...
/**
 * eva_http_response_from_request:
				...
				gint64          length)
{
  ...
  if (request == NULL)
    {
      eva_http_header_set_version (header_response, 1, 0);
    }
  else
    {
      header_response->connection_type = header_request->connection_type;
      eva_http_header_set_version (header_response,
  ...
  if (length < 0)
    {
      if (   request != NULL
          ...
          && status_code == EVA_HTTP_STATUS_OK)
        header_response->transfer_encoding_type = EVA_HTTP_TRANSFER_ENCODING_CHUNKED;
      else
        header_response->connection_type = EVA_HTTP_CONNECTION_CLOSE;
    }
