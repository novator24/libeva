...
int main(int argc, char **argv)
{
  ...
  g_option_context_set_summary (op_context, "eva-analyze-successive-memdumps");
  g_option_context_add_main_entries (op_context, op_entries, NULL);
  if (!g_option_context_parse (op_context, &argc, &argv, &error))
    eva_fatal_user_error ("error parsing command-line options: %s", error->message);
  ...
  for (i = 0; i < n_files; i++)
    {
      ...
      if (fp == NULL)
        eva_fatal_user_error ("opening %s failed: %s",
	                      files[i], g_strerror (errno));
      do
        {
          if (!fgets (buf, sizeof (buf), fp))
            eva_fatal_user_error ("file %s: unexpected eof", files[i]);
          line_no++;
        }
...
next_block_start:
      ...
      if (sscanf (buf, "%u bytes allocated in %u blocks from:",
        ...
        eva_fatal_user_error ("error parsing line %u from %s",
                              line_no, files[i]);
...
done_file:
  ...
  if (!eva_mkdir_p (dir_name, 0755, &error))
    eva_fatal_user_error ("error making directory %s: %s", dir_name, error->message);
  ...
  for (i = 0; i < G_N_ELEMENTS (subdirs); i++)
    {
      ...
      if (!eva_mkdir_p (tmp_fname, 0755, &error))
  ...
  if (system (tmp_fname) != 0)
    eva_fatal_user_error ("error running gnuplot");
  g_free (tmp_fname);
  g_printerr (" done.\n");
