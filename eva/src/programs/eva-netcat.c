...
#include "../ssl/evastreamlistenerssl.h"
...
usage (void)
{
  g_printerr ("eva-netcat --client HOST PORT\n"
              "eva-netcat --server PORT\n"
              "\n"
...
connect_to_stdio (GskStream *str, gboolean terminate_on_input_death)
{
  ...
  if (terminate_on_input_death)
    g_object_weak_ref (G_OBJECT (in), (GWeakNotify) eva_main_quit, NULL);
  g_object_unref (in);
  g_object_unref (out);
}
...
setup_client (void)
{
  ...
  str = eva_stream_new_connecting (client_hostport.address, &error);
  if (str == NULL)
  ...
  g_object_weak_ref (G_OBJECT (str), (GWeakNotify) eva_main_quit, NULL);
  g_object_unref (str);
}
