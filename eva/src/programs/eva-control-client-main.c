#include "../control/evacontrolclient.h"
#include "../config.h"
#include "../evainit.h"
#include <stdlib.h>
#include <stdio.h>
...
static void usage ()
{
  ...
  eva_control_client_print_command_line_usage (EVA_CONTROL_CLIENT_OPTIONS_DEFAULT);
  exit (1);
}
...
int main(int argc, char **argv)
{
  ...
  if (!eva_control_client_parse_command_line_args
      (cc, &argc, &argv, EVA_CONTROL_CLIENT_OPTIONS_DEFAULT))
    return 0;
  for (i = 1; i < argc; i++)
    {
  ...
  if (!eva_control_client_has_address (cc))
    usage ();

  for (;;)
    {
      char *prompt = eva_control_client_get_prompt_string (cc);
