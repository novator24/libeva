 ...
 */
gboolean
eva_pass_fd_make_pair    (int          *sender_fd_out,
                          int          *receiver_fd_out,
                          GError      **error)
{
  int fds[2];
 ...
 */
int
eva_pass_fd_make_sender  (GError      **error)
{
  int fd;
retry_socket:
 ...
 * returns: the file-descriptor, or -1 on error.
 */
int      eva_pass_fd_bind_receiver(const char   *path,
                                   GError      **error)
{
  int fd = eva_pass_fd_make_sender (error);
 ...
 * which was allocated either via eva_pass_fd_make_pair()
 * or eva_pass_fd_bind_receiver().
 *
 ...
 * readable (using eva_main_loop_add_io()).
 *
 ...
 */
int
eva_pass_fd_receive      (int           receiver_fd,
                          ...
                          GError      **error)
{
  ...
  eva_fd_set_nonblocking (fd);  /* kinda unclear if this is a good idea */
