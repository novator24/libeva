                                  ...
                                  guint             restart_exit_code)
{
  ...
  if (flags & EVA_DAEMONIZE_FORK)
    do_fork = TRUE;
  else
    do_fork = FALSE;
  if (flags & EVA_DAEMONIZE_SUPPORT_RESTART_EXIT_CODE)
    {
...
void
eva_daemonize_parse_options (int              *argc_inout,
                             char           ***argv_inout)
{
  gint i;
