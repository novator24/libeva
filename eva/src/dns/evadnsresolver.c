			   ...
			   gpointer            handle_result_data)
{
  ...
  for (;;)
	...
	answer = list_search (additional, host, rrtype);
      if (answer != NULL)
	{
	  if (answer->type == EVA_DNS_RR_HOST_ADDRESS)
	    {
      ...
      answer = list_search (answers, host, EVA_DNS_RR_CANONICAL_NAME);
      if (answer == NULL)
	answer = list_search (authority, host, EVA_DNS_RR_CANONICAL_NAME);
      if (answer == NULL)
	answer = list_search (additional, host, EVA_DNS_RR_CANONICAL_NAME);
      if (answer == NULL)
	break;
			  ...
			  GDestroyNotify         destroy)
{
  ...
  return eva_dns_resolver_resolve (resolver,
				   TRUE,
				   &question_list,
...
/* --- type implementation --- */
GType
eva_dns_resolver_get_type (void)
{
  static GType type = 0;
  if (type == 0)
    {
...
void
eva_dns_resolver_add_name_resolver_iface (GType type)
{
