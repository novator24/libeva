...
/* XXX: lowercase_string and LOWER_CASE_COPY_ON_STACK are
        copied from evadnsimplementations.c, hmm. */
...
struct _RRList
{
  ...
  /* Whether the entry is a negative entry.
     ...
     EVA_DNS_RESPONSE_ERROR_NAME_ERROR error code. */
  guint is_negative : 1;
...
/* For debugging, implement ASSERT_INVARIANTS(rr_cache),
   ...
   NOTE: this is unused unless you specify --eva-debug=dns
			     ...
			     gulong                   cur_time)
{
  ...
  g_return_val_if_fail (record->type != EVA_DNS_RR_WILDCARD, NULL);
		      ...
		      GskDnsResourceClass      query_class)
{
  ...
  if (query_type != EVA_DNS_CLASS_WILDCARD
 ...
 * negative information registered with eva_dns_rr_cache_add_negative()
			     ...
			     GskDnsResourceClass      query_class)
{
  ...
  for (at = g_hash_table_lookup (rr_cache->owner_to_rr_list, lc_owner);
      ...
      if (!at->is_negative)
	{
	  ...
	  /*
	  else if (at->rr.type == EVA_DNS_RR_CANONICAL_NAME)
	   ...
	   at = at->owner_next)
	{
	  if (!at->is_negative)
	      ...
	      if (record_matches_query (&at->rr, query_type, query_class))
		{
	      ...
	      else if ((flags & EVA_DNS_RR_CACHE_LOOKUP_DEREF_CNAMES) != 0
                    && at->rr.type == EVA_DNS_RR_CANONICAL_NAME)
		{
		  if (cname_table == NULL)
		    {
...
/**
 * eva_dns_rr_cache_lock:
 * @rr_cache: a resource-record cache.
 * @record: a record obtained from eva_dns_rr_cache_lookup_one()
 * or eva_dns_rr_cache_lookup_list().
 *
			       ...
			       GskDnsResourceRecord    *record)
{
  ...
  eva_dns_rr_cache_unlock (rr_cache, record);
}

/**
 * eva_dns_rr_cache_get_addr:
 * @rr_cache: a resource-record cache.
			     ...
			     GskSocketAddressIpv4   **address)
{
  ...
  addr = eva_socket_address_ipv4_new (record->rdata.a.ip_address, EVA_DNS_PORT);
  *address = EVA_SOCKET_ADDRESS_IPV4 (addr);
  return TRUE;
}


/**
 * eva_dns_rr_cache_get_ns_addr:
 * @rr_cache: a resource-record cache.
...
retry:
  record_list = eva_dns_rr_cache_lookup_list (rr_cache,
					      host,
  ...
  for (at = record_list; at != NULL; at = at->next)
	...
	*ns_name_out = record->owner;
	 ...
	 if anything is added to the cache */
      if (eva_dns_rr_cache_get_addr (rr_cache, record->rdata.domain_name, address_out))
	{
			      ...
			      GskDnsResourceClass      query_class)
{
  ...
  for (at = owner_list; at != NULL; at = at->owner_next)
    {
      if ( (at->rr.type == EVA_DNS_RR_WILDCARD
         || at->rr.type == query_type)
       &&  (at->rr.record_class == EVA_DNS_CLASS_WILDCARD
  ...
  for (;;)
    {
      ...
      next_to_expire = eva_g_tree_min (rr_cache->rr_list_by_expire_time);
			   ...
			   int              lineno)
{
  ...
  /* XXX: we need some kind of funny allocator for these
          temporary cases! */
  rr = eva_dns_rr_text_parse (command,
			      *last_owner_in_out,
			      origin,
			      &err_message,
			      NULL);
...
/* ???: it'd be nice to make public functions for this, probably:
      eva_path_get_file_relative
      eva_path_get_dir_relative

   (this is eva_path_get_file_relative) */
static char *
...
fail:
  ...
  while (stack != NULL)
    stack = pop_include_stack (stack);
  g_set_error (error, EVA_G_ERROR_DOMAIN, EVA_ERROR_RESOLVER_FORMAT,
...
/**
 * eva_dns_rr_cache_parse_etc_hosts_line:
 * @rr_cache: a resource-record cache.
				      ...
				      const char         *text)
{
  ...
  if (!eva_dns_parse_ip_address (&text, addr))
    return FALSE;
  EVA_SKIP_WHITESPACE (text);
  ...
  rr = eva_dns_rr_new_a (canon_name, 1000, addr, NULL);
  in_cache = eva_dns_rr_cache_insert (rr_cache, rr, FALSE, tv.tv_sec);
  eva_dns_rr_cache_mark_user (rr_cache, in_cache);
  eva_dns_rr_free (rr);
  ...
  while (*text != 0)
    {
      ...
      rr = eva_dns_rr_new_cname (tmp, 1000, canon_name, NULL);
      in_cache = eva_dns_rr_cache_insert (rr_cache, rr, FALSE, tv.tv_sec);
      eva_dns_rr_cache_mark_user (rr_cache, in_cache);
      eva_dns_rr_free (rr);
      g_free (tmp);
...
/**
 * eva_dns_rr_cache_parse_etc_hosts:
 * @rr_cache: a resource-record cache.
				    ...
				    gboolean           may_be_missing)
{
  ...
  while (fgets (buf, sizeof (buf), fp) != NULL)
      ...
      if (nl == NULL)
	{
      ...
      *nl = '\0';
      if (!eva_dns_rr_cache_parse_etc_hosts_line (rr_cache, buf))
	{
