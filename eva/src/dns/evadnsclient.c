...
#define DEBUG_PRINT_MESSAGE(mess)	eva_dns_dump_message_fp(mess, stderr)
#else
#define DEBUG(args...)
...
maybe_remove_timeout_source (ClientTask *task)
{
  if (task->timeout_source != NULL)
    {
      eva_source_remove (task->timeout_source);
...
static void
eva_dns_client_task_unref (ClientTask *task)
{
  g_return_if_fail (task->ref_count > 0);
  --(task->ref_count);
  if (task->ref_count == 0)
	...
	eva_dns_rr_cache_unref (task->rr_cache);
      g_free (task);
    }
}
				 ...
				 gpointer           task_data)
{
  ...
  task->cancelled = 1;
  eva_dns_client_task_unref (task);
}
  ...
  if (message == NULL)
    {
      ...
      message = eva_dns_message_new (msg_id, TRUE);
	       ...
	       MessageLists         *lists)
{
  ...
  else if (record->type == EVA_DNS_RR_NAME_SERVER)
 ...
 * to eva_dns_resolver_resolve must discern
 * these cases.
 */
static void
eva_dns_client_task_succeed (ClientTask *task)
{
...
static void
eva_dns_client_task_fail (ClientTask *task,
			  GError             *error)
{
	      ...
	      DnsQueryData         *query_data)
{
  ...
  if (name_server == NULL)
	...
	eva_dns_client_task_use_conf_nameservers (task);
  ...
  eva_dns_client_transmit (task->client,
			   name_server->address,
			   message);
		...
		GskSocketAddressIpv4  *address)
{
  ...
  for (rv = task->first_ns; rv != NULL; rv = rv->next_ns)
    if (eva_socket_address_equals (address, rv->address))
      return rv;
  rv = eva_dns_name_server_info_alloc ();
...
try_local_cache_or_proceed (ClientTask  *task)
{
  ...
  if (rr_cache == NULL && !task->stub_resolver)
    rr_cache = task->rr_cache = eva_dns_rr_cache_new (0, 0);
...
restart_with_local_cache:
      ...
      if (rr_cache != NULL)
	{
	  if (query_type == EVA_DNS_RR_WILDCARD)
	    results = eva_dns_rr_cache_lookup_list (rr_cache,
						    name,
						    query_type,
						    query_class);
	  else
	  ...
	  if (eva_dns_rr_cache_is_negative (rr_cache, 
					    name,
					    query_type,
					    query_class))
	    {
	  ...
	  if (query_type != EVA_DNS_RR_CANONICAL_NAME
	   && query_type != EVA_DNS_RR_WILDCARD)
	    {
	      record = eva_dns_rr_cache_lookup_one (rr_cache,
						    name,
      ...
      if (task->stub_resolver)
	{
	  ...
	  eva_dns_message_append_question (message, q);
      ...
      for (found_ns_index = 0; found_ns_index < n_pieces; found_ns_index++)
	{
	  ...
	  DEBUG ("calling eva_dns_rr_cache_get_ns_addr(..,%s,..)", pieces[found_ns_index]);
	  if (eva_dns_rr_cache_get_ns_addr (rr_cache,
	...
	for (i = 0; i < found_ns_index; i++)
	  {
	     ...
	     */
	    q = eva_dns_question_new (pieces[i],
  ...
  if (ns_to_dns_message == NULL)
    {
      ...
      eva_dns_client_task_succeed (task);
      eva_dns_client_task_unref (task);
      return;
    }
			...
			GskDnsResolverHints          *hints)
{
  ...
  task->client = client;
  task->message_id = eva_dns_client_generate_message_id (client);
  ...
  if (task->rr_cache != NULL)
    eva_dns_rr_cache_ref (task->rr_cache);
			       ...
			       GskDnsRRCache        *rr_cache)
{
  ...
  if (record->type == EVA_DNS_RR_HOST_ADDRESS
   || record->type == EVA_DNS_RR_HOST_ADDRESS_IPV6)
    {
      const char *query_name = question->query_name;
      ...
      while (query_name != NULL)
	{
	  ...
	  GSList *ns_list;
	  do {
	    ns_list = eva_dns_rr_cache_lookup_list (rr_cache, qn,
                                                    EVA_DNS_RR_NAME_SERVER,
                                                    EVA_DNS_CLASS_INTERNET);
	    qn = strchr (qn, '.');
	    if (qn)
				 ...
				 guint                 cur_time)
{
  while (rr_list != NULL)
      ...
      if (!check_rr_authority (task->client, address, record, cur_time))
	{
      ...
      record = eva_dns_rr_cache_insert (task->rr_cache,
					record,
					is_authoritative,
					cur_time);
      task->locked_records = g_slist_prepend (task->locked_records, record);
      eva_dns_rr_cache_lock (task->rr_cache, record);
      rr_list = rr_list->next;
    }
}
		     ...
		     GskDnsMessage        *message)
{
  ...
  for (iteration = 0; iteration < 3; iteration++)
	...
	//DEBUG ("task_handle_message: task->client->rr_cache=%p; record=%s", task->client->rr_cache, eva_dns_rr_text_to_str (record, NULL));	/*XXX: memory leak */

	/* Add that RR to our cache. */
	if (task->client->rr_cache != NULL)
	  record = eva_dns_rr_cache_insert (task->rr_cache, record,
	...
	else if (record->type == EVA_DNS_RR_CANONICAL_NAME)
	  {
  ...
  if (!one_answer_was_relevant)
      ...
      if (found == NULL)
	{
	  ...
	  eva_g_debug ("Received useless message with matching ID.");
#if 0
	  g_message ("MESSAGE WAS:");
	  eva_dns_dump_message_fp (message, stderr);
	  g_message ("QUESTIONS WERE:");
	  g_slist_foreach (task->questions, (GFunc) eva_dns_dump_question_fp, stderr);
	  g_message ("ANSWERED QUESTIONS WERE:");
	  g_slist_foreach (task->answered_questions, (GFunc) eva_dns_dump_question_fp, stderr);
#endif
	  return;
	}
    }
			 ...
			 GskDnsMessage        *message)
{
  ...
  if (eva_debug_flags & EVA_DEBUG_DNS)
    {
      char *a = eva_socket_address_to_string (EVA_SOCKET_ADDRESS (address));
  ...
  if (client->first_outgoing_packet == NULL)
    {
      ...
      if (!eva_packet_queue_write (client->packet_queue, packet, &error))
	{
	  if (error != NULL)
	    {
	      eva_dns_client_fail_all_tasks (client, error);
	      g_error_free (error);
	      return;
	    }
	  /* fall-through */
  ...
  if (client->is_blocking_write)
    {
      ...
      eva_io_unblock_write (EVA_IO (client->packet_queue));
    }
}
			  ...
			  gpointer       data)
{
  ...
  message = eva_dns_message_parse_data ((const guint8 *) packet->data,
					packet->len, &used);
#ifdef EVA_DEBUG
  if (eva_debug_flags & EVA_DEBUG_DNS)
    {
      char *a = eva_socket_address_to_string (packet->src_address);
  ...
  client_handle_message (client, message, 
			 EVA_SOCKET_ADDRESS_IPV4 (address));
			           ...
			           gpointer       data)
{
  ...
  eva_dns_client_fail_all_tasks (EVA_DNS_CLIENT (data), error);
  g_error_free (error);
  return FALSE;
}
...
{
  g_object_unref (EVA_DNS_CLIENT (c)->packet_queue);
}
...
eva_dns_client_finalize (GObject *object)
{
  ...
  if (client->rr_cache != NULL)
    eva_dns_rr_cache_unref (client->rr_cache);
...
eva_dns_client_init (GskDnsClient *dns_client)
{
  ...
  dns_client->main_loop = eva_main_loop_default ();
...
eva_dns_client_class_init (GskDnsClientClass *dns_client_class)
{
  ...
  object_class->finalize = eva_dns_client_finalize;
}

GType eva_dns_client_get_type()
{
  ...
  if (!dns_client_type)
	...
	NULL		/* value_table */
      ...
      {
	(GInterfaceInitFunc) eva_dns_client_resolver_init,
	...
	NULL			/* interface_data */
      ...
      eva_dns_resolver_add_name_resolver_iface (dns_client_type);
    }
					      ...
					      GskDnsClientFlags   flags)
{
  ...
  eva_io_trap_readable (g_object_ref (packet_queue),
		        handle_queue_is_readable,
		        handle_queue_is_readable_shutdown,
		        client,
		        unref_packet_queue);
  eva_io_trap_writable (g_object_ref (packet_queue),
		        handle_queue_is_writable,
		        handle_queue_is_writable_shutdown,
		        client,
		        unref_packet_queue);
  client->is_blocking_write = 1;
  eva_io_block_write (EVA_IO (packet_queue));
  client->stub_resolver = (flags & EVA_DNS_CLIENT_STUB_RESOLVER) ? 1 : 0;
  client->rr_cache = rr_cache;
  if (rr_cache != NULL)
    eva_dns_rr_cache_ref (rr_cache);
  else
    client->rr_cache = eva_dns_rr_cache_new (EVA_DNS_MAX_CACHE_BYTES,
                                             EVA_DNS_MAX_CACHE_RECORDS);
  return client;
}

/**
 * eva_dns_client_add_searchpath:
			      ...
			      GskSocketAddressIpv4   *address)
{
  ...
  if (ns_info != NULL)
    return;
  ns_info = eva_dns_name_server_info_alloc ();
...
/**
 * eva_dns_client_parse_resolv_conf_line:
				      ...
				      const char         *text)
{
  ...
  if (g_strncasecmp (text, "nameserver", 10) == 0)
    {
      ...
      text += 10;
      EVA_SKIP_WHITESPACE (text);
      if (!eva_dns_parse_ip_address (&text, ip_address))
	return FALSE;
      address = eva_socket_address_ipv4_new (ip_address, EVA_DNS_PORT);
      eva_dns_client_add_ns (client, EVA_SOCKET_ADDRESS_IPV4 (address));
      return TRUE;
    }
  if (g_strncasecmp (text, "search", 6) == 0)
      ...
      while (*text != 0)
	{
	  ...
	  EVA_SKIP_NONWHITESPACE (end);
	  if (end == text)
	    break;
	  memcpy (tmp, text, end - text);
	  tmp[end - text] = 0;

	  eva_dns_client_add_searchpath (client, tmp);
...
/**
 * eva_dns_client_parse_resolv_conf:
 * @client: the client which should add the resolv.conf information
				  ...
				  gboolean            may_be_missing)
{
  ...
  while (fgets (buf, sizeof (buf), fp) != NULL)
      ...
      if (nl == NULL)
	{
      ...
      *nl = '\0';
      if (!eva_dns_client_parse_resolv_conf_line (client, buf))
	{
...
/**
 * eva_dns_client_parse_system_files:
...
ip_permission_table_new ()
{
  ...
  ip_perm_table->sockaddr_to_perm_addr
    = g_hash_table_new (eva_socket_address_hash,
			    ...
			    guint              cur_time)
{
  ...
  while ((data = eva_g_tree_min (table->by_expire_time)) != NULL)
    {
