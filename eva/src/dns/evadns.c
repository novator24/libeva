	...
	if ((str) != NULL)					\
	  {							\
	    ...
	    if (len > 255)					\
	      len = 255;					\
	    len8 = (guint8) len;				\
	    eva_buffer_append (buffer, &len8, 1);		\
	    eva_buffer_append (buffer, (str), len8);		\
	  }							\
	else							\
	  eva_buffer_append_char (buffer, 0);			\
      }G_STMT_END
...
#define EVA_SKIP_DIGITS(char_ptr)	EVA_SKIP_CHAR_TYPE(char_ptr, isdigit)
...
/* TEST ONLY */
#if 1
#define PARSE_FAIL(note)	eva_g_debug ("NOTE: parse error: %s", note)
#else
...
eva_dns_message_free (EvaDnsMessage *eva_dns_message)
{
  ...
  G_LOCK (eva_dns_message_chunk);
  g_mem_chunk_free (eva_dns_message_chunk, eva_dns_message);
  G_UNLOCK (eva_dns_message_chunk);
}
  ...
  while (!stop)
    {
      ...
      guint piece_len;
      gchar *str;
      guint component_offset = eva_buffer_iterator_offset (iterator);
      if (eva_buffer_iterator_read (iterator, &tmp_len, 1) != 1)
	return NULL;
      piece_len = tmp_len;
      ...
      if ((piece_len >> 6) == 3)
	{
	  guint8 ptr_byte_2;
	  guint offset;
	  if (eva_buffer_iterator_read (iterator, &ptr_byte_2, 1) != 1)
	    return NULL;
      ...
      else
	{
	  ...
	  if (eva_buffer_iterator_read (iterator,
					tmp_buf,
					piece_len) != piece_len)
	    {
	      /* XXX: parse error? */
		          ...
		          int                max_iterate)
{
  ...
  if (eva_buffer_iterator_read (iterator, &len, 1) != 1)
    return NULL;
  max_iterate--;
  if (len == 0)
    return NULL;
  if (len > max_iterate)
    return NULL;
  if (eva_buffer_iterator_read (iterator, buf, len) != len)
    return NULL;
  buf[len] = 0;
		   ...
		   int                max_iterate)
{
  ...
  while (max_iterate > 0)
	...
	break;
      if (len > max_iterate)
	break;
      if (eva_buffer_iterator_read (iterator, buf + out_len, len) != len)
	return NULL;
      out_len += len;
      max_iterate -= len;
    }
 ...
 */
gboolean
eva_test_domain_name_validity (const char *domain_name)
{
			...
			EvaDnsMessage *allocator)
{
  ...
  rv->type = EVA_DNS_RR_HOST_ADDRESS;
			...
			EvaDnsMessage *allocator)
{
  ...
  if (!eva_test_domain_name_validity (owner))
    return NULL;
  rv = eva_dns_rr_new_generic (allocator, owner, ttl);
  rv->type = EVA_DNS_RR_HOST_ADDRESS_IPV6;
			...
			EvaDnsMessage *allocator)
{
  ...
  if (!eva_test_domain_name_validity (owner)
   || ! eva_test_domain_name_validity (canonical_name))
    return NULL;
  rv = eva_dns_rr_new_generic (allocator, owner, ttl);
  rv->type = EVA_DNS_RR_CANONICAL_NAME;
			...
			EvaDnsMessage *allocator)
{
  ...
  if (!eva_test_domain_name_validity (owner)
   || !eva_test_domain_name_validity (mail_host))
    return NULL;
  rv = eva_dns_rr_new_generic (allocator, owner, ttl);
  rv->type = EVA_DNS_RR_MAIL_EXCHANGE;
			...
			EvaDnsMessage *allocator)
{
  ...
  if (!eva_test_domain_name_validity (owner)
   ...
   || !eva_test_domain_name_validity (rname))
    return NULL;
  rv = eva_dns_rr_new_generic (allocator, owner, ttl);
  rv->type = EVA_DNS_RR_START_OF_AUTHORITY;
...
eva_dns_rr_free        (EvaDnsResourceRecord *record)
{
  ...
  switch (record->type)
    {
    case EVA_DNS_RR_HOST_ADDRESS:
    case EVA_DNS_RR_HOST_ADDRESS_IPV6:
      break;
    case EVA_DNS_RR_NAME_SERVER:
    case EVA_DNS_RR_CANONICAL_NAME:
    case EVA_DNS_RR_POINTER:
      g_free (record->rdata.domain_name);
      break;
    case EVA_DNS_RR_HOST_INFO:
      g_free (record->rdata.hinfo.os);
      g_free (record->rdata.hinfo.cpu);
      break;
    case EVA_DNS_RR_MAIL_EXCHANGE:
      g_free (record->rdata.mx.mail_exchange_host_name);
      break;
    case EVA_DNS_RR_START_OF_AUTHORITY:
      g_free (record->rdata.soa.mname);
      g_free (record->rdata.soa.rname);
      break;
    case EVA_DNS_RR_TEXT:
      g_free (record->rdata.txt);
      break;
    case EVA_DNS_RR_WELL_KNOWN_SERVICE:
      g_warning ("XXX: unimplemented");
      break;
    case EVA_DNS_RR_ZONE_TRANSFER:
      g_warning ("XXX: unimplemented");
      break;
    case EVA_DNS_RR_ZONE_MAILB:
      g_warning ("XXX: unimplemented");
      break;
    case EVA_DNS_RR_WILDCARD:
      break;
    default:
			...
			EvaDnsMessage        *allocator)
{
  switch (record->type)
    {
    case EVA_DNS_RR_HOST_ADDRESS:
      return eva_dns_rr_new_a (record->owner,
			       record->time_to_live,
			       record->rdata.a.ip_address,
			       allocator);
    case EVA_DNS_RR_HOST_ADDRESS_IPV6:
      return eva_dns_rr_new_aaaa (record->owner,
			          record->time_to_live,
			          record->rdata.aaaa.address,
			          allocator);
    case EVA_DNS_RR_NAME_SERVER:
      return eva_dns_rr_new_ns (record->owner,
				record->time_to_live,
				record->rdata.domain_name,
				allocator);
    case EVA_DNS_RR_CANONICAL_NAME:
      return eva_dns_rr_new_cname (record->owner,
				   record->time_to_live,
				   record->rdata.domain_name,
				   allocator);
    case EVA_DNS_RR_POINTER:
      return eva_dns_rr_new_ptr (record->owner,
				 record->time_to_live,
				 record->rdata.domain_name,
				 allocator);
    case EVA_DNS_RR_HOST_INFO:
      return eva_dns_rr_new_hinfo (record->owner,
				   record->time_to_live,
				   record->rdata.hinfo.cpu,
				   record->rdata.hinfo.os,
				   allocator);
    case EVA_DNS_RR_MAIL_EXCHANGE:
      return eva_dns_rr_new_mx (record->owner,
				record->time_to_live,
				record->rdata.mx.preference_value,
				record->rdata.mx.mail_exchange_host_name,
				allocator);
    case EVA_DNS_RR_START_OF_AUTHORITY:
      return eva_dns_rr_new_soa (record->owner,
				 record->time_to_live,
				 ...
				 allocator);
    case EVA_DNS_RR_TEXT:
      return eva_dns_rr_new_txt (record->owner,
				 record->time_to_live,
				 record->rdata.txt,
				 allocator);
    case EVA_DNS_RR_WELL_KNOWN_SERVICE:
    ...
    case EVA_DNS_RR_ZONE_MAILB:
      ...
      return NULL;
    case EVA_DNS_RR_WILDCARD:
      return eva_dns_rr_new_wildcard (record->owner,
				      record->time_to_live,
				      allocator);
    default:
		...
		EvaDnsMessage     *message)
{
  ...
  if (eva_buffer_iterator_read (iterator, qarray, 4) != 4)
    return NULL;
  qarray[0] = GUINT16_FROM_BE (qarray[0]);
  qarray[1] = GUINT16_FROM_BE (qarray[1]);

  question = eva_dns_question_new (NULL, qarray[0], qarray[1], message);
		       ...
		       EvaDnsMessage     *message)
{
  ...
  if (eva_buffer_iterator_read (iterator, header, 10) != 10)
    return NULL;
  ...
  rr = eva_dns_rr_new_generic (message, owner, ttl);
  rr->type = type;
  ...
  switch (rr->type)
    {
    case EVA_DNS_RR_HOST_ADDRESS:
      if (class == EVA_DNS_CLASS_INTERNET)
	{
	  if (rdlength != 4)
	    {
	...
	if (eva_buffer_iterator_read (iterator, &preference_value, 2) != 2)
	  return NULL;
    ...
    case EVA_DNS_RR_HOST_INFO:
      {
	guint init_offset = eva_buffer_iterator_offset (iterator);
	guint used_len;
	rr->rdata.hinfo.cpu
	  = parse_char_single_string (iterator, message, rdlength);
	used_len
	  = eva_buffer_iterator_offset (iterator) - init_offset;
	rr->rdata.hinfo.os
	...
	rr->rdata.soa.rname = parse_domain_name (iterator, message);
        if (rr->rdata.soa.rname == NULL)
          return NULL;
	used_len = eva_buffer_iterator_offset (iterator) - init_offset;
	if (rdlength < used_len + 20)
	  return NULL;
	if (eva_buffer_iterator_read (iterator, intervals, 20) != 20)
	  return NULL;
	...
	g_assert(eva_buffer_iterator_offset (iterator) - init_offset == rdlength);
      }
      break;
			       ...
			       guint        *num_bytes_parsed)
{
  ...
  if (num_bytes_parsed != NULL)
    *num_bytes_parsed = eva_buffer_iterator_offset (&iterator);
  return message;

fail:
  if (message != NULL)
    eva_dns_message_unref (message);
  return NULL;
}
				              ...
				              guint        *bytes_used_out)
{
  ...
  message = eva_dns_parse_buffer_internal (&buffer, &len);
			     ...
			     const char    *str)
{
  ...
  while (*component != 0)
	...
	len = MAX_COMPONENT_LENGTH;
      eva_buffer_append_char (ser_info->buffer, len);
      eva_buffer_append (ser_info->buffer, component, len);
  ...
  if (offset == 0)
    eva_buffer_append_char (ser_info->buffer, 0);
  else
    {
      eva_buffer_append_char (ser_info->buffer, (offset >> 8) | 0xc0);
      eva_buffer_append_char (ser_info->buffer, offset & 0xff);
    }
}
			  ...
			  gpointer    ser_data)
{
  ...
  eva_buffer_append (ser_info->buffer, data, 4);
}
		    ...
		    const char *str)
{
  ...
  if (len > 63)
    len = 63;
  eva_buffer_append_char (buffer, len);
  eva_buffer_append (buffer, str, len);
}
		    ...
		    gpointer    ser_data)
{
  ...
  switch (rr->type)
    {
    case EVA_DNS_RR_HOST_ADDRESS:
      if (rr->record_class == EVA_DNS_CLASS_INTERNET)
	{
	  data[4] = GUINT16_TO_BE (4);
	  eva_buffer_append (buffer, data, 10);
	  eva_buffer_append (buffer, rr->rdata.a.ip_address, 4);
	}
      else
	{
      ...
      if (rr->record_class == EVA_DNS_CLASS_INTERNET)
	{
	  ...
	  eva_buffer_append (buffer, data, 10);
	  eva_buffer_append (buffer, rr->rdata.aaaa.address, 16);
	}
      else
	{
	  g_warning ("cannot serialize DnsClasses beside `INTERNET'");
	...
	eva_buffer_append (buffer, data, 10);
	eva_buffer_drain (buffer, &tmp_buffer);
	...
	ser_info->init_buffer_size = old_init_buf_size
				   - buffer->size
				   - 10;
	eva_buffer_append (&tmp_buffer, &pref, 2);
	...
	compress_string (ser_info, rr->rdata.soa.rname);
	{
	  ...
	  eva_buffer_append (buffer, int_buf, sizeof (int_buf));
	}
	...
	while (remaining > 0)
	  {
	    ...
	    eva_buffer_append_char (buffer, to_write);
	    eva_buffer_append (buffer, text, to_write);
	    remaining -= to_write;
	    text += to_write;
	  }
	break;
		               ...
		               gboolean       compress)
{
...

  {
    ...
    eva_buffer_append (buffer, dns_header, 12);
  }
...
eva_dns_message_to_packet (EvaDnsMessage *message)
{
  ...
  gpointer slab;
  eva_buffer_construct (&buffer);
  eva_dns_message_write_buffer (message, &buffer, TRUE);
  size = buffer.size;
  slab = g_malloc (size);
...
/**
 * eva_dns_message_remove_question:
				 ...
				 EvaDnsQuestion       *question)
{
  ...
  eva_dns_question_free (question);
}

/**
 * eva_dns_message_remove_answer:
 * @message: a DNS message.
				 ...
				 EvaDnsResourceRecord *answer)
{
  ...
  eva_dns_rr_free (answer);
}

/**
 * eva_dns_message_remove_auth:
 * @message: a DNS message.
				 ...
				 EvaDnsResourceRecord *auth)
{
  ...
  eva_dns_rr_free (auth);
}

/**
 * eva_dns_message_remove_addl:
 * @message: a DNS message.
				 ...
				 EvaDnsResourceRecord *addl)
{
  ...
  eva_dns_rr_free (addl);
}

/* --- refcounting messages --- */
/**
 * eva_dns_message_unref:
...
eva_dns_message_unref           (EvaDnsMessage        *message)
{
  ...
  if (message->ref_count == 0)
    {
      g_slist_foreach (message->questions, (GFunc) eva_dns_question_free, NULL);
      g_slist_free (message->questions);
      g_slist_foreach (message->answers, (GFunc) eva_dns_rr_free, NULL);
      g_slist_free (message->answers);
      g_slist_foreach (message->authority, (GFunc) eva_dns_rr_free, NULL);
      g_slist_free (message->authority);
      g_slist_foreach (message->additional, (GFunc) eva_dns_rr_free, NULL);
      g_slist_free (message->additional);
      if (message->offset_to_str != NULL)
	g_hash_table_destroy (message->offset_to_str);
      eva_dns_message_free (message);
    }
}
 ...
 */
gboolean
eva_dns_parse_ip_address (const char **pat,
		          guint8      *ip_addr_out)
{
  ...
  if (at == endp)
    return FALSE;
  EVA_SKIP_WHITESPACE (endp);
  *pat = endp;
  return TRUE;
}

/**
 * eva_dns_parse_ipv6_address:
 * @pat: pointer which starts at a numeric IP address.  *@pat
 * will be updated to past the IP address.
 * @ip_addr_out: the 4-byte IP address.
 *
 ...
 */
gboolean
eva_dns_parse_ipv6_address (const char **pat,
		            guint8      *ip_addr_out)
{
  guint i;
  const char *at = *pat;
...
#define TEST_STRING(str, rv)			\
  ...
  switch (*type_str)
    {
    case 'a': case 'A':
      TEST_STRING("a", EVA_DNS_RR_HOST_ADDRESS);
      TEST_STRING("aaaa", EVA_DNS_RR_HOST_ADDRESS_IPV6);
      TEST_STRING("axfr", EVA_DNS_RR_ZONE_TRANSFER);
      break;

    case 'n': case 'N':
      TEST_STRING ("ns", EVA_DNS_RR_NAME_SERVER);
      break;

    case 'h': case 'H':
      TEST_STRING ("hinfo", EVA_DNS_RR_HOST_INFO);
      break;

    case 'c': case 'C':
      TEST_STRING ("cname", EVA_DNS_RR_CANONICAL_NAME);
      break;

    case 'm': case 'M':
      TEST_STRING ("mx", EVA_DNS_RR_MAIL_EXCHANGE);
      break;

    case 'p': case 'P':
      TEST_STRING ("ptr", EVA_DNS_RR_POINTER);
      break;

    case 's': case 'S':
      TEST_STRING ("soa", EVA_DNS_RR_START_OF_AUTHORITY);
      break;

    case 'w': case 'W':
      TEST_STRING ("wks", EVA_DNS_RR_WELL_KNOWN_SERVICE);
      break;

    case '*':
      TEST_STRING ("*", EVA_DNS_RR_WILDCARD);
      break;
    }
#undef TEST_STRING
  /*eva_g_debug ("Unknown RRTYPE parsed from text: %s", type_str);*/
  return FALSE;
}
...
#define TEST_STRING(second_chars, rv)		\
  ...
  switch (*class_str)
    {
    case 'i': case 'I':
      TEST_STRING ("nN", EVA_DNS_CLASS_INTERNET);
      break;
    case 'c': case 'C':
      TEST_STRING ("hH", EVA_DNS_CLASS_CHAOS);
      break;
    case 'h': case 'H':
      TEST_STRING ("sS", EVA_DNS_CLASS_HESIOD);
      break;
    }
#undef TEST_STRING
  /*eva_g_debug ("Unknown RRCLASS parsed from text: %s", class_str);*/
  return FALSE;
}

/**
 * eva_dns_rr_text_parse:
			   ...
			   EvaDnsMessage    *allocator)
{
  ...
  switch (rtype)
    {
    case EVA_DNS_RR_HOST_ADDRESS:
      if (rclass == EVA_DNS_CLASS_INTERNET)
	{
	  guint8 ip_address[4];
	  if (!eva_dns_parse_ip_address (&at, ip_address))
	    {
	      if (err_msg != NULL)
		*err_msg = g_strdup ("error parsing IP address for A record");
	      goto fail;
	    }
	  rr = eva_dns_rr_new_a (owner, ttl, ip_address, allocator);
	}
      else
	{
	...
	{
	  guint8 address[16];
	  if (!eva_dns_parse_ipv6_address (&at, address))
	    {
	      if (err_msg != NULL)
		*err_msg = g_strdup ("error parsing IP address for AAAA record");
	      goto fail;
	    }
	  rr = eva_dns_rr_new_aaaa (owner, ttl, address, allocator);
	}
      else
	{
	  /* TODO: maybe add a way to register more classes externally? */
	...
	if (rtype == EVA_DNS_RR_NAME_SERVER)
	  rr = eva_dns_rr_new_ns (owner, ttl, name, allocator);
	else if (rtype == EVA_DNS_RR_CANONICAL_NAME)
	  rr = eva_dns_rr_new_cname (owner, ttl, name, allocator);
	else if (rtype == EVA_DNS_RR_POINTER)
	  rr = eva_dns_rr_new_ptr (owner, ttl, name, allocator);
      }
      break;
	...
	*err_msg = g_strdup ("HINFO text messages not supported");
	...
	SUFFIXED_CUT_ONTO_STACK (name, at, endp, origin);
	rr = eva_dns_rr_new_mx (owner, ttl, preference, name, allocator);
	break;
      }
	...
	rr = eva_dns_rr_new_soa (owner, ttl, mname, rname,
    ...
    case EVA_DNS_RR_TEXT:
      rr = eva_dns_rr_new_txt (owner, ttl, at, allocator);
      break;
...
eva_resource_record_type_to_string (EvaDnsResourceRecordType type)
{
  switch (type)
    {
    case EVA_DNS_RR_HOST_ADDRESS:       return "A";
    case EVA_DNS_RR_HOST_ADDRESS_IPV6:  return "AAAA";
    case EVA_DNS_RR_NAME_SERVER:        return "NS";
    case EVA_DNS_RR_CANONICAL_NAME:     return "CNAME";
    case EVA_DNS_RR_HOST_INFO:          return "HINFO";
    case EVA_DNS_RR_MAIL_EXCHANGE:      return "MX";
    case EVA_DNS_RR_POINTER:            return "PTR";
    case EVA_DNS_RR_START_OF_AUTHORITY: return "SOA";
    case EVA_DNS_RR_TEXT:               return "TXT";
    case EVA_DNS_RR_WELL_KNOWN_SERVICE: return "WKS";
    case EVA_DNS_RR_ZONE_TRANSFER:      return "AXFR";
    case EVA_DNS_RR_ZONE_MAILB:         return "MAILB";
    case EVA_DNS_RR_WILDCARD:           return "*";
...
eva_resource_record_class_to_string (EvaDnsResourceClass class)
{
  switch (class)
    {
    case EVA_DNS_CLASS_INTERNET:        return "IN";
    case EVA_DNS_CLASS_CHAOS:           return "CH";
    case EVA_DNS_CLASS_HESIOD:          return "HS";
    case EVA_DNS_CLASS_WILDCARD:        return "*";
    default:                            return "UNKNOWN-RCLASS";
    }
}

/**
 * eva_dns_rr_text_to_str:
		       ...
		       const char           *last_owner)
{
  ...
  /* append type */
  g_string_append (rv, eva_resource_record_type_to_string (rr->type));
  g_string_append_c (rv, ' ');

  /* append class */
  g_string_append (rv, eva_resource_record_class_to_string (rr->record_class));
  g_string_append_c (rv, ' ');
...
eva_dns_question_text_to_str(EvaDnsQuestion       *question)
{
  ...
  enum_str = eva_resource_record_class_to_string (question->query_class);
  g_string_append (rv, enum_str);
  g_string_append_c (rv, ' ');
  enum_str = eva_resource_record_type_to_string (question->query_type);
  g_string_append (rv, enum_str);
			   ...
			   const char           *last_owner)
{
  ...
  eva_buffer_append_string (out_buffer, txt);
  eva_buffer_append_char (out_buffer, '\n');
  g_free (txt);
}
			 ...
			 FILE          *fp)
{
  ...
  switch (message->error_code)
    {
    case EVA_DNS_RESPONSE_ERROR_NONE:
      error_str = "NO ERROR";
      break;
    case EVA_DNS_RESPONSE_ERROR_FORMAT:
      error_str = "FORMAT ERROR";
      break;
    case EVA_DNS_RESPONSE_ERROR_SERVER_FAILURE:
      error_str = "SERVER FAILURE";
      break;
    case EVA_DNS_RESPONSE_ERROR_NAME_ERROR:
      error_str = "NAME ERROR";
      break;
    case EVA_DNS_RESPONSE_ERROR_NOT_IMPLEMENTED:
      error_str = "NOT IMPLEMENTED ERROR";
      break;
    case EVA_DNS_RESPONSE_ERROR_REFUSED:
      error_str = "REFUSED";
      break;
    }
  ...
  if (message->questions != NULL)
    fprintf (fp, "\nQuestions:\n");
  g_slist_foreach (message->questions, (GFunc) eva_dns_dump_question_fp, fp);
