...
static void eva_base64_decode_internal   (char            *dst,
				          guint           *dst_len,
				          int              dst_len_max,
                                          const char      *src,
					  gsize            src_len)
 ...
 * Should be EVA_BASE64_GET_MAX_DECODED_LEN(@src) long at least.
...
eva_base64_decode_alloc (const char      *src)
{
  ...
  eva_base64_decode_internal ((char*)rv->data, &rv_size, rv->len, src, len);
  if (rv->len != rv_size)
    {
 ...
 * EVA_BASE64_GET_ENCODED_LEN(@src_len) bytes long.
 ...
 */
char *
eva_base64_encode_alloc (const char      *src,
			 gssize           src_len)
{
  unsigned raw_len = (src_len >= 0) ? (gsize)src_len : strlen (src);
  unsigned enc_len = EVA_BASE64_GET_ENCODED_LEN (raw_len);
  char *buf = g_malloc (enc_len + 1);
  eva_base64_encode (buf, src, raw_len);
  buf[enc_len] = '\0';
  return buf;
}
