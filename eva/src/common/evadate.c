   ...
   && isalpha (date_str[2]) && islower (date_str[2]))
    {
      if (isspace (date_str[3])
       && (allowed & EVA_DATE_FORMAT_ANSI_C) == EVA_DATE_FORMAT_ANSI_C)
        return parse_ansi_c (date_str, tm_out, tzoffset_out);
      if (date_str[3] == ','
       && (allowed & EVA_DATE_FORMAT_1123) == EVA_DATE_FORMAT_1123)
        return parse_1123 (date_str, tm_out, tzoffset_out);
      if (isalpha (date_str[3]) && islower (date_str [3])
       && (allowed & EVA_DATE_FORMAT_1036) == EVA_DATE_FORMAT_1036)
        return parse_1036 (date_str, tm_out, tzoffset_out);
    }
  if (isdigit (date_str[0])
   && isdigit (date_str[1])
   && (allowed & EVA_DATE_FORMAT_ISO8601) == EVA_DATE_FORMAT_ISO8601)
    {
				    ...
				    EvaDateFormatMask  formats_allowed)
{
  ...
  struct tm new_tm;
#endif
  if (!eva_date_parse (date_str, &tm_out, &tz_offset, formats_allowed))
    return FALSE;
				    ...
				    EvaDateFormatMask  format)
{
  ...
  if ((format & EVA_DATE_FORMAT_1123) == EVA_DATE_FORMAT_1123)
    {
      g_snprintf (date_str_out, date_str_max_len,
                  "%s, %02d %s %d %02d:%02d:%02d GMT",
  ...
  if ((format & EVA_DATE_FORMAT_ANSI_C) == EVA_DATE_FORMAT_ANSI_C)
    {
      g_snprintf (date_str_out, date_str_max_len,
  ...
  if ((format & EVA_DATE_FORMAT_ISO8601) == EVA_DATE_FORMAT_ISO8601)
    {
  ...
  g_warning ("eva_date_print: EVA_DATE_FORMAT_* expected");
  g_snprintf (date_str_out, date_str_max_len, "error");
}

/**
 * eva_date_print_timet:
				    ...
				    EvaDateFormatMask  format)
{
  ...
  eva_date_print (tm_out, date_str_out, date_str_max_len, format);
}
