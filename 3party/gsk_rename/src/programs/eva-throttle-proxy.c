...
#define CURRENT_SECOND() (eva_main_loop_default ()->current_time.tv_sec)
...
update_write_block (Side   *side)
{
  ...
  if (old_val && !val)
    eva_io_unblock_write (side->write_side);
  else if (!old_val && val)
    eva_io_block_write (side->write_side);
}
  ...
  if (side->throttled && !was_throttled)
    {
      ...
      EVA_LIST_APPEND (GET_THROTTLED_LIST (), side);
    }
  else if (!side->throttled && was_throttled)
    {
      /* remove from throttled list */
      EVA_LIST_REMOVE (GET_THROTTLED_LIST (), side);
    }

  if (old_val && !val)
    eva_io_unblock_read (side->read_side);
  else if (!old_val && val)
    eva_io_block_read (side->read_side);
}
...
connection_unref (EvaThrottleProxyConnection *conn)
{
  if (--(conn->ref_count) == 0)
    {
      EVA_LIST_REMOVE (GET_CONNECTION_LIST (), conn);
      eva_buffer_destruct (&conn->upload.buffer);
      eva_buffer_destruct (&conn->download.buffer);
      g_free (conn);
    }
}
                      ...
                      gpointer data)
{
  Side *side = data;
  GError *error = NULL;
  guint written = eva_stream_write_buffer (stream, &side->buffer, &error);
  if (error)
    {
  ...
  if (written == 0 && side->read_side == NULL && side->buffer.size == 0)
    {
      update_write_block (side);
      if (half_shutdowns)
        eva_io_write_shutdown (side->write_side, NULL);
      else
        eva_io_shutdown (EVA_IO (side->write_side), NULL);
    }
  return TRUE;
}
                            ...
                            gpointer   data)
{
  ...
  if (side->read_side)
    {
      if (half_shutdowns)
        eva_io_read_shutdown (side->read_side, NULL);
      else
        eva_io_shutdown (EVA_IO (side->read_side), NULL);
    }
  return FALSE;
}
                      ...
                      gpointer data)
{
  ...
  nread = eva_stream_read (stream, tmp, max_read, &error);
  if (error != NULL)
    {
  ...
  eva_buffer_append (&side->buffer, tmp, nread);
           ...
           guint      max_xfer_per_second)
{
  ...
  side->last_xfer_second = eva_main_loop_default ()->current_time.tv_sec;
  side->xferred_in_last_second = 0;
  eva_buffer_construct (&side->buffer);
  ...
  eva_io_trap_readable (read_side,
  ...
  eva_io_trap_writable (write_side,
                ...
                GError      **error)
{
  ...
  conn->ref_count = 1;
  EVA_LIST_APPEND (GET_CONNECTION_LIST (), conn);
...
unblock_timer_func (gpointer data)
{
  ...
  eva_main_loop_add_timer_absolute (eva_main_loop_default (),
                    ...
                    gpointer        data)
{
  ...
  eva_buffer_printf (&buffer, "<br>%u connections accepted.\n",
                     n_connections_accepted);
  eva_buffer_printf (&buffer, "<br>%"G_GUINT64_FORMAT" bytes read.\n",
                     n_bytes_read_total);
  eva_buffer_printf (&buffer, "<br>%"G_GUINT64_FORMAT" bytes written.\n",
                     n_bytes_written_total);
  eva_buffer_printf (&buffer, "<h1>Connections</h1>\n");
  eva_buffer_printf (&buffer, "<table>\n"
  ...
  for (conn = first_conn; conn; conn = conn->next)
    {
      ...
      eva_buffer_printf (&buffer, "</tr>\n");
    }
  eva_buffer_printf (&buffer, "</table>\n</body>\n</html>\n");
  response = eva_http_response_from_request (request, 200, buffer.size);
  eva_http_header_set_content_type (response, "text");
  eva_http_header_set_content_subtype (response, "html");
  stream = eva_memory_buffer_source_new (&buffer);
      ...
      if (g_str_has_prefix (argv[i], "--bind="))
        {
          ...
          if (g_ascii_isdigit (bind_str[0]))
            {
              bind_addr = eva_socket_address_ipv4_new (eva_ipv4_ip_address_any,
                                                       atoi (bind_str));
            }
          else
            {
              bind_addr = eva_socket_address_local_new (bind_str);
            }
        }
      else if (g_str_has_prefix (argv[i], "--bind-status="))
        {
          ...
          if (g_ascii_isdigit (bind_str[0]))
            {
              bind_status_addr = eva_socket_address_ipv4_new (eva_ipv4_ip_address_any,
                                                       atoi (bind_str));
            }
          else
            {
              bind_status_addr = eva_socket_address_local_new (bind_str);
            }
        }
          ...
          if (colon != NULL && strchr (server_str, '/') == NULL)
            {
              ...
              server_addr = eva_socket_address_symbolic_ipv4_new (host, port);
              g_free (host);
            }
          else
            {
              /* unix */
              server_addr = eva_socket_address_local_new (server_str);
            }
        }
  ...
  if (bind_status_addr != NULL)
    {
      ...
      if (!eva_http_content_listen (content, bind_status_addr, &error))
        g_error ("error listening: %s", error->message);
    }

  eva_main_loop_add_timer_absolute (eva_main_loop_default (),
                                    unblock_timer_func, NULL, NULL,
                                    eva_main_loop_default ()->current_time.tv_sec + 1, 0);
