...
usage ()
{
  g_printerr ("usage: eva-escape MODE\n\n"
...
int main(int argc, char **argv)
{
  ...
  if (strcmp (argv[1], "escape") == 0)
    {
      char *out = eva_escape_memory (array->data, array->len);
      fputs (out, stdout);
      fputc ('\n', stdout);
      g_free (out);
    }
  else if (strcmp (argv[1], "unescape") == 0)
    {
      ...
      out = eva_unescape_memory ((char *) array->data, FALSE, NULL, &len, &error);
      if (out == NULL)
    ...
    {
      char *out = eva_escape_memory_hex (array->data, array->len);
      fputs (out, stdout);
      fputc ('\n', stdout);
      g_free (out);
    }
  else if (strcmp (argv[1], "unescape-hex") == 0)
    {
      ...
      if (array->len > 0 && array->data[array->len - 1] == '\n')
        g_byte_array_set_size (array, array->len - 1);
      out = eva_unescape_memory_hex ((char*)array->data, array->len, &len, &error);
      if (out == NULL)
        g_error ("error unescaping: %s", error->message);
      fwrite (out, len, 1, stdout);
      g_free (out);
