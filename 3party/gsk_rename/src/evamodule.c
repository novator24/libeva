...
EvaCompileContext *eva_compile_context_new ()
{
  ...
  rv->cflags = g_string_new (EVA_CFLAGS " " EVA_COMPILE_ONLY_FLAG);
  rv->ldflags = g_string_new (EVA_LD_SHLIB_FLAGS);
...
/**
 * eva_compile_context_add_cflags:
...
/**
 * eva_compile_context_add_ldflags:
 * @context: the compilation context.
...
/**
 * eva_compile_context_set_tmp_dir:
 * @context: the compilation context.
...
/**
 * eva_compile_context_set_verbose:
 * @context: the compilation context.
...
eva_compile_context_free       (EvaCompileContext *context)
{
  ...
  eva_g_ptr_array_foreach (context->packages, (GFunc) g_free, NULL);
                      ...
                      GError           **error)
{
  ...
  cmd_str = g_string_new (EVA_PKGCONFIG);
  ...
  if (pclose_rv != 0)
    {
      if (pclose_rv < 255)
        g_set_error (error, EVA_G_ERROR_DOMAIN, EVA_ERROR_COMPILE,
