...
/**
 * eva_http_request_parse_first_line:
  ...
  if (verb_length < 3 || verb_length > 4)
    {
      ...
      return EVA_HTTP_REQUEST_FIRST_LINE_ERROR;
    }
  if (verb_length == 3 && g_strncasecmp (line, "PUT", 3) == 0)
    request->verb = EVA_HTTP_VERB_PUT;
    ...
    request->verb = EVA_HTTP_VERB_POST;
  else if (verb_length == 4 && g_strncasecmp (line, "HEAD", 4) == 0)
    request->verb = EVA_HTTP_VERB_HEAD;
  else
    {
      g_set_error (error, EVA_G_ERROR_DOMAIN,
                   EVA_ERROR_HTTP_PARSE,
  ...
  if (g_ascii_strncasecmp (line + at, "HTTP/", 5) == 0 && isdigit (line[at + 5]))
    {
      ...
      return EVA_HTTP_REQUEST_FIRST_LINE_FULL;
    }
  else
    {
      return EVA_HTTP_REQUEST_FIRST_LINE_SIMPLE;
    }
}

/**
 * eva_http_response_process_first_line:
                                      ...
                                      const char      *line)
{
  ...
  if (g_strncasecmp (line, "http/", 5) != 0)
    {
      g_set_error (&header->g_error, EVA_G_ERROR_DOMAIN,
                   EVA_ERROR_HTTP_PARSE,
              ...
              gpointer       data)
{
  guint offset = GPOINTER_TO_UINT (data);
  time_t t;
  if (!eva_date_parse_timet (value, &t, EVA_DATE_FORMAT_HTTP))
    {
      return FALSE;
    }
              ...
              gpointer       data)
{
  ...
  eva_http_header_set_string (header, p_str, value);
  return TRUE;
}
...
handle_transfer_encoding (EvaHttpHeader *header,
			  ...
			  gpointer data)
{
  eva_http_header_set_transfer_encoding_string (header, value);
  return TRUE;
}
...
handle_content_encoding  (EvaHttpHeader *header,
			  ...
			  gpointer data)
{
  eva_http_header_set_content_encoding_string (header, value);
  return TRUE;
}
...
handle_connection (EvaHttpHeader *header,
		   ...
		   gpointer data)
{
  eva_http_header_set_connection_string (header, value);
  return TRUE;
}
                               ...
                               gpointer       data)
{
  ...
  eva_http_response_set_cache_control (EVA_HTTP_RESPONSE (header), control);
  return TRUE;
        }
                              ...
                              gpointer       data)
{
  ...
  eva_http_request_set_cache_control (EVA_HTTP_REQUEST (header), control);
  return TRUE;
}
...
handle_pragma    (EvaHttpHeader *header,
                  ...
                  gpointer       data)
{
  eva_http_header_add_pragma (header, value);
  return TRUE;
}
  ...
  while (TRUE)
    {
      ...
      for (;;)
        {
          ...
          if (*at == 0)
            break;
          this_key_start = at;
          EVA_SKIP_CHAR_TYPE (at, is_cookie_key_name_char);
          this_key_end = at;
          ...
          EVA_SKIP_WHITESPACE (this_value_start);
          if (*this_value_start == '"')
            {
              this_value_start++;
...

      {
        ...
        cookie = eva_http_cookie_new (key,
                                      value,
                                      path,
                                      domain,
                                      expire_date,
...
parse_charset (const char **pstr)
{
  ...
  if (!parse_str_quality (pstr, &charset, &quality))
    return NULL;
  rv = eva_http_char_set_new (charset, quality);
  g_free (charset);
  return rv;
}
  ...
  while (*value != '\0')
    {
      ...
      eva_http_request_add_charsets (EVA_HTTP_REQUEST (header), set);
    }
  return TRUE;
}
...
parse_content_encoding (const char **pstr)
{
  ...
  if (strncasecmp (str, "identity", 8) == 0)
    {
      encoding = EVA_HTTP_CONTENT_ENCODING_IDENTITY;
    }
  else if (strncasecmp (str, "gzip", 4) == 0)
    {
      encoding = EVA_HTTP_CONTENT_ENCODING_GZIP;
    }
  else if (strncasecmp (str, "compress", 8) == 0)
    {
      encoding = EVA_HTTP_CONTENT_ENCODING_COMPRESS;
    }
  else
    {
      encoding = EVA_HTTP_CONTENT_ENCODING_UNRECOGNIZED;
    }
  ...
  *pstr = str;
  return eva_http_content_encoding_set_new (encoding, quality);
}
  ...
  while (*value != '\0')
    {
      ...
      eva_http_request_add_content_encodings (EVA_HTTP_REQUEST (header), encoding);
    }
  return TRUE;
}
...
parse_transfer_encoding (const char **pstr)
{
  ...
  if (strncasecmp (str, "none", 8) == 0)
    {
      encoding = EVA_HTTP_TRANSFER_ENCODING_NONE;
    }
  else if (strncasecmp (str, "chunked", 7) == 0)
    {
      encoding = EVA_HTTP_TRANSFER_ENCODING_CHUNKED;
    }
  else
    {
      encoding = EVA_HTTP_TRANSFER_ENCODING_UNRECOGNIZED;
    }
  ...
  *pstr = str;
  return eva_http_transfer_encoding_set_new (encoding, quality);
}
  ...
  while (*value != '\0')
    {
      ...
      if (encoding == NULL)
        HEADER_HANDLER_FAIL(("error parsing encoding from %s", value));
      eva_http_request_add_transfer_encodings (EVA_HTTP_REQUEST (header), encoding);
    }
  return TRUE;
}
		      ...
		      gpointer data)
{
  if (strcmp (value, "none") == 0
   || strcmp (value, "") == 0)
    ...
    {
      eva_http_header_add_accepted_range (header, EVA_HTTP_RANGE_BYTES);
      return TRUE;
    }
  else
    return FALSE;
  ...
  for (;;)
    {
      ...
      EVA_SKIP_CHAR_TYPE (value, IS_8ALPHA);
      if (value[0] == '-')
	{
	  value++;
	  EVA_SKIP_CHAR_TYPE (value, IS_8ALPHA);
      ...
      if (*value == ';')
	{
      ...
      set = eva_http_language_set_new (language, quality);
      if (rv == NULL)
	rv = set;
      else
	{
...
parse_media_type (const char **pstr)
{
  ...
  *slash = '\0';
  ...
  if (options_ptr != NULL && *options_ptr == 'q')
    {
      options_ptr++;
      EVA_SKIP_WHITESPACE (options_ptr);
      if (*options_ptr == '=')
        {
          options_ptr++;
  ...
  *pstr = end_str;
  return eva_http_media_type_set_new (type, subtype, quality);
}
  ...
  while (*value != '\0')
    {
      ...
      eva_http_request_add_media (EVA_HTTP_REQUEST (header), media_type);
    }
  return TRUE;
}
...
{
  char *init = str;
  EVA_SKIP_WHITESPACE (str);
  if (*str != '"')
    {
      const char *start = str;
      EVA_SKIP_NONWHITESPACE (str);
	    ...
	    gpointer    data)
{
  ...
  if (!isdigit (*value))
    return FALSE;
  EVA_HTTP_RESPONSE (header)->age = atoi (value);
  return TRUE;
}
	      ...
	      gpointer       data)
{
  ...
  while (TRUE)
    {
      ...
      buf[i] = 0;
      if (strcmp (buf, "GET") == 0)
        val |= (1 << EVA_HTTP_VERB_GET);
      else if (strcmp (buf, "POST") == 0)
        val |= (1 << EVA_HTTP_VERB_POST);
      else if (strcmp (buf, "PUT") == 0)
        val |= (1 << EVA_HTTP_VERB_PUT);
      else if (strcmp (buf, "HEAD") == 0)
        val |= (1 << EVA_HTTP_VERB_HEAD);
      else if (strcmp (buf, "OPTIONS") == 0)
        val |= (1 << EVA_HTTP_VERB_OPTIONS);
      else if (strcmp (buf, "DELETE") == 0)
        val |= (1 << EVA_HTTP_VERB_DELETE);
      else if (strcmp (buf, "TRACE") == 0)
        val |= (1 << EVA_HTTP_VERB_TRACE);
      else if (strcmp (buf, "CONNECT") == 0)
        val |= (1 << EVA_HTTP_VERB_CONNECT);
      else
        {
  ...
  EVA_HTTP_RESPONSE (header)->allowed_verbs = val;
  return TRUE;
}
  ...
  if (!eva_date_parse_timet (value, &response->expires, EVA_DATE_FORMAT_HTTP))
    {
      ...
      eva_http_header_set_string (header, &response->expires_str, value);
    }
  return TRUE;
}
		       ...
		       gpointer data)
{
  ...
  /* XXX: eva_base64_decode() should be guint8 on the output, sigh */
  if (eva_base64_decode ((char*)response->md5sum, 16, value, -1) == 16)
                                      ...
                                      GError                **error)
{
  ...
  if ((flags & EVA_HTTP_CONTENT_TYPE_PARSE_ADDL) == 0)
		     ...
		     gpointer data)
{
  ...
  if (!eva_http_content_type_parse (value,
                                    EVA_HTTP_CONTENT_TYPE_PARSE_ADDL,
                                    &type_info,
                                    &error))
    {
      g_warning ("eva_http_content_type_parse failed: %s",
                 error->message);
      g_error_free (error);
      return FALSE;
    }
  header->has_content_type = 1;
  eva_http_header_set_string_len (header, &header->content_type, type_info.type_start, type_info.type_len);
  eva_http_header_set_string_len (header, &header->content_subtype, type_info.subtype_start, type_info.subtype_len);
  eva_http_header_set_string_len (header, &header->content_charset, type_info.charset_start, type_info.charset_len);
		    ...
		    gpointer data)
{
  ...
  if (isdigit (*value))
  ...
  else
    {
      ...
      if (!eva_date_parse_timet (value, &response->retry_after, EVA_DATE_FORMAT_HTTP))
        {
...
/* NOTES:
    ...
    - IS_NONQUOTED_ATTR is intended for use with EVA_SKIP_CHAR_TYPE().
 */
...
#define CUT_TOKEN_MAYBE_QUOTED(var_name)             \
  G_STMT_START{                                      \
    if (*at == '"')                                  \
      ...
      {                                              \
        start = at;                                  \
        EVA_SKIP_CHAR_TYPE (at, IS_NONQUOTED_ATTR);  \
        CUT_TOKEN (var_name);                        \
        if (*at)                                     \
  ...
  while (*at)
    {
      ...
      EVA_SKIP_CHAR_TYPE (key_end, IS_ATTR_NAME_CHAR);
      at = key_end;
      EVA_SKIP_WHITESPACE (at);
      if (*at != '=')
        {
...
#undef IS_KEY
  ...
  if (g_ascii_strcasecmp (auth_scheme, "Basic") == 0)
    {
      rv = eva_http_authenticate_new_basic (realm);
    }
  else if (g_ascii_strcasecmp (auth_scheme, "Digest") == 0)
    {
      /* XXX: missing qop and stale ??? */
      rv = eva_http_authenticate_new_digest (realm, domain, nonce,
                                             opaque, algorithm);
    }
  else
    {
  ...
  if (g_ascii_strcasecmp (auth_scheme, "basic") == 0)
    {
      ...
      guint decoded_max_len = EVA_BASE64_GET_MAX_DECODED_LEN (base64_encoded_len);
      char *decoded = g_alloca (decoded_max_len + 1);
      char *colon;
      guint decoded_len = eva_base64_decode (decoded, decoded_max_len,
      ...
      *colon = 0;
      return eva_http_authorization_new_basic (decoded, colon + 1);
    }
  if (g_ascii_strcasecmp (auth_scheme, "digest") != 0)
    {
      return eva_http_authorization_new_unknown (auth_scheme, at);
    }
...
#undef IS_KEY
  ...
  rv = eva_http_authorization_new_digest (realm, domain, nonce, opaque,
		         ...
		         gpointer data)
{
  ...
  eva_http_authenticate_unref (auth);
  return TRUE;
}
		      ...
		      gpointer data)
{
  ...
  eva_http_authorization_unref (auth);
  return TRUE;
}
...
/**
 * eva_http_header_get_parser_table:
 ...
 */
GHashTable *
eva_http_header_get_parser_table (gboolean is_request)
{
	      ...
	      gboolean *line_mem_from_stack)
{
  ...
  eva_buffer_iterator_peek (start, *line_mem, len);
                             ...
                             GError        **error)
{
  ...
  eva_buffer_iterator_construct (&iterator, input);
  newline = iterator;
  if (!eva_buffer_iterator_find_char (&newline, '\n'))
    return NULL;
  snip_between (&iterator, &newline, &line_size, &line_mem, &line_mem_from_stack);
  header_type = is_request ? EVA_TYPE_HTTP_REQUEST : EVA_TYPE_HTTP_RESPONSE;
...
#define ERROR_RETURN()                          \
  ...
  if (is_request)
    {
      switch (eva_http_request_parse_first_line (EVA_HTTP_REQUEST (rv),
                                                 line_mem, 
                                                 error))
        {
        ...
        case EVA_HTTP_REQUEST_FIRST_LINE_SIMPLE:
          if (!line_mem_from_stack)
            g_free (line_mem);
          eva_buffer_discard (input, eva_buffer_iterator_offset (&newline) + 1);
          return rv;
    ...
    {
      if (!eva_http_response_process_first_line (EVA_HTTP_RESPONSE (rv), line_mem))
        {
          if (rv->g_error)
            {
  ...
  eva_buffer_iterator_skip (&newline, 1);
  iterator = newline;

  parser_table = eva_http_header_get_parser_table (is_request);
  for (;;)
    {
      ...
      if (parser == NULL)
	{
	  ...
	  eva_http_header_add_misc (rv, line_mem, at);
	}
      else
	{
