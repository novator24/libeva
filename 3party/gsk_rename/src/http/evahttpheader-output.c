...
static GEnumClass *eva_http_content_encoding_class = NULL;
static GEnumClass *eva_http_transfer_encoding_class = NULL;
static GEnumClass *eva_http_verb_class = NULL;

static inline void
init_classes (void)
{
  eva_http_connection_class = g_type_class_ref (EVA_TYPE_HTTP_CONNECTION);
  eva_http_content_encoding_class = g_type_class_ref (EVA_TYPE_HTTP_CONTENT_ENCODING);
  eva_http_transfer_encoding_class = g_type_class_ref (EVA_TYPE_HTTP_TRANSFER_ENCODING);
  eva_http_verb_class = g_type_class_ref (EVA_TYPE_HTTP_VERB);
}
			 ...
			 gpointer               data)
{
  ...
  GEnumValue *enum_value = g_enum_get_value (eva_http_verb_class, verb);
  ...
  for (at = set; at != NULL; at = at->next)
    {
	  ...
	  default:
	    /* XXX: error handling */
	    g_warning ("eva_http_content_encoding_set_append_list: "
  ...
  for (at = set; at != NULL; at = at->next)
    {
	  ...
	  default:
	    /* XXX: error handling */
	    g_warning ("eva_http_transfer_encoding_set_append_list: "
	    		"unknown encoding %d", at->encoding);
	    break;
	}
      if (at->quality >= 0.0)
...
{
  /* XXX: implement this!!! */
  //eva_buffer_append (buffer, "Accept-Ranges: ", 15);
}
...
{
  /* XXX: implement this!!! */
  // eva_buffer_append (buffer, "Accept: ", 8);
}
...
#define MAYBE_ADD_VERB(verb)				\
  G_STMT_START{						\
    if ((allowed & (1 << EVA_HTTP_VERB_ ## verb)) != 0)	\
      {							\
	if (len > 7)					\
	  {						\
	    strcpy (tmp + len, ", ");			\
		 ...
		 gpointer               print_data)
{
  ...
  eva_date_print_timet (date, tmp + len, sizeof (tmp) - len, EVA_DATE_FORMAT_1123);
  (*print_func) (tmp, print_data);
}
		      ...
		      gpointer                print_data)
{
  ...
  if (eva_http_verb_class == NULL)
    init_classes ();
  if (EVA_IS_HTTP_REQUEST (http_header))
    request = EVA_HTTP_REQUEST (http_header);
  if (EVA_IS_HTTP_RESPONSE (http_header))
    response = EVA_HTTP_RESPONSE (http_header);
...
#define MAYBE_PRINT_DATE(object, member, tag)				\
  ...
  if (http_header->http_minor_version < 1)
    {
      if (http_header->connection_type == EVA_HTTP_CONNECTION_KEEPALIVE)
	{
	  /* XXX: TODO: we should support this.  see RFC 2068 for HTTP/1.0 things... */
	  g_warning ("!! WE DON'T SUPPORT KEEPALIVE FOR HTTP/1.0");
	}
      else if (http_header->connection_type == EVA_HTTP_CONNECTION_CLOSE)
   ...
   * Content-Encoding:
   */
  if (http_header->content_encoding_type == EVA_HTTP_CONTENT_ENCODING_IDENTITY)
    type = NULL;
  else if (http_header->content_encoding_type == EVA_HTTP_CONTENT_ENCODING_UNRECOGNIZED)
    type = http_header->content_encoding;
  else
    {
      enum_value = g_enum_get_value (eva_http_content_encoding_class, http_header->content_encoding_type);
   ...
   * Transfer-Encoding:
   */
  if (http_header->transfer_encoding_type == EVA_HTTP_TRANSFER_ENCODING_NONE)
    type = NULL;
  else if (http_header->transfer_encoding_type == EVA_HTTP_TRANSFER_ENCODING_UNRECOGNIZED)
    type = http_header->unrecognized_transfer_encoding;
  else
    {
      enum_value = g_enum_get_value (eva_http_transfer_encoding_class, http_header->transfer_encoding_type);
      type = enum_value ? enum_value->value_nick : NULL;
    }
  ...
  if (http_header->accepted_range_units != NULL)
    eva_http_range_set_append_list (http_header->accepted_range_units,
                                    print_func, print_data);
  ...
  if (request)
	...
	eva_http_char_set_append_list (request->accept_charsets,
				       print_func, print_data);
      /* Add `Accept-Encoding' */
      if (request->accept_content_encodings != NULL)
	eva_http_content_encoding_set_append_list (request->accept_content_encodings,
					            print_func, print_data);
      /* Add `Accept-Encoding' */
      if (request->accept_transfer_encodings != NULL)
	eva_http_transfer_encoding_set_append_list (request->accept_transfer_encodings,
					            print_func, print_data);

      /* Add `Accept-Languages' */
      if (request->accept_languages != NULL)
	eva_http_language_set_append_list (request->accept_languages,
					   print_func, print_data);

      /* Add `Accept' */
      if (request->accept_media_types != NULL)
	eva_http_media_type_set_append_list (request->accept_media_types,
					     print_func, print_data);
      /* Add `If-Match' */
      if (request->had_if_match)
	eva_http_append_if_matches (request->if_match, print_func, print_data);
	...
	print_allowed_verb (response->allowed_verbs, print_func, print_data);
      ...
      if (response->has_md5sum)
        {
	  char encoded[50 + EVA_BASE64_GET_ENCODED_LEN (16)];
	  strcpy (encoded, "Content-MD5: ");
	  eva_base64_encode (encoded + 13, (char*)response->md5sum, 16);
	  encoded[13 + EVA_BASE64_GET_ENCODED_LEN (16)] = 0;
