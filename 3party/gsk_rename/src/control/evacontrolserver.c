                 ...
                 const char *error_message)
{
  ...
  content = eva_memory_source_new_printf ("ERROR!!! (%s)\n\n"
                                          "%s\n",
                                          status_name, error_message);
  eva_http_server_respond (server, request, response, content);
                ...
                gpointer        data)
{
    ...
    }
  {
    ...
    eva_http_header_set_content_subtype (response, "plain");
    if (output == NULL)
      output = eva_memory_source_static_string ("");
    eva_http_server_respond (server, request, response, output);
    g_object_unref (response);
...
return_string:
  {
    ...
    *output = eva_memory_slab_source_new (mem, len, g_free, mem);
    g_strfreev (path_pieces);
    return TRUE;
  }
}
                     ...
                     GError **error)
{
  ...
  switch (eva_control_server_stat (server, argv[1]))
    {
      case EVA_CONTROL_SERVER_FILE_RAW:
        if (!eva_control_server_peek_raw_file (data, argv[1], &content, &content_length))
          {
            g_set_error (error, EVA_G_ERROR_DOMAIN,
                         EVA_ERROR_FILE_NOT_FOUND,
        ...
        *output = eva_memory_slab_source_new (copy, content_length, g_free, copy);
        return TRUE;
      case EVA_CONTROL_SERVER_FILE_VIRTUAL:
        {
          guint8 *content;
          guint length;
          GDestroyNotify release;
          gpointer release_data;
          if (!eva_control_server_get_vfile_contents (server, argv[1],
                                                      &content, &length,
          ...
          *output = eva_memory_slab_source_new (content, length, release, release_data);
          return TRUE;
        }
      case EVA_CONTROL_SERVER_FILE_DIR:
        g_set_error (error, EVA_G_ERROR_DOMAIN,
                     EVA_ERROR_FILE_NOT_FOUND,
                     "cat: %s was a directory", argv[1]);
        return FALSE;
      case EVA_CONTROL_SERVER_FILE_NOT_EXIST:
        g_set_error (error, EVA_G_ERROR_DOMAIN,
                     EVA_ERROR_FILE_NOT_FOUND,
...
eva_control_server_new (void)
{
  ...
  eva_http_content_add_handler (server->content, &id, handler, EVA_HTTP_CONTENT_REPLACE);
...
/**
 * eva_control_server_add_command:
...
{
  char *contents = eva_log_ring_buffer_get (vfile_data);
...
/**
 * eva_control_server_set_logfile_v:
                                  ...
                                  const EvaControlServerLogDomain *domains)
{
  ...
  for (i = 0; i < n_log_domains; i++)
    eva_log_trap_ring_buffer (domains[i].domain, domains[i].levels,
                              ring_buffer, NULL);
}
                                  ...
                                  ...)
{
  ...
  eva_control_server_set_logfile_v (server, path, ring_buffer_size,
                                    n_domains, domains);
}
...
/**
 * eva_control_server_delete_file:
...
/**
 * eva_control_server_delete_directory:
 * @server: the server which have the virtual file removed from.
 * @path: the virtual path for the file's location.
 * @error: place to store the error if failure occurs.
 *
                                ...
                                const char       *path)
{
  ...
  if (p[0] == NULL)
    {
      g_free (p);
      return EVA_CONTROL_SERVER_FILE_DIR;
    }
  ...
  for (at = p; at[1]; at++)
    {
      ...
      if (sub == NULL)
        {
          g_strfreev (p);
          return EVA_CONTROL_SERVER_FILE_NOT_EXIST;
        }
      node = sub;
    }
  if (node->files == NULL)
    ...
    for (i = 0; i < node->files->len; i++)
      {
        ...
        if (strcmp (fn->name, *at) == 0)
          {
            g_strfreev (p);
            return fn->is_virtual ? EVA_CONTROL_SERVER_FILE_VIRTUAL
...
/**
 * eva_control_server_peek_raw_file:
