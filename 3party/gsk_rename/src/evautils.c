...
/**
 * eva_unlock_dir:
 * @lock_rv: the return-value from eva_lock_dir()
 * @error: optional error return location.
 *
 * Unlock a directory locked with eva_lock_dir().
 *
 ...
 */
char *
eva_escape_memory (gconstpointer    data,
		   guint            len)
{
 ...
 */
gpointer
eva_unescape_memory (const char *quoted,
 ...
 */
guint8 *
eva_unescape_memory_hex (const char  *str,
                         gssize       len,
                         gsize       *length_out,
                         GError     **error)
{
 ...
 */
gssize
eva_readn (guint fd, void *buf, gsize len)
{
  gsize rv = 0;
  while (rv < len)
    {
 ...
 */
gssize
eva_writen (guint fd, const void *buf, gsize len)
{
  gsize rv = 0;
  while (rv < len)
    {
