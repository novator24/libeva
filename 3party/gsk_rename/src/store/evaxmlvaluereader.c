					 ...
					 ...)
{
  ...
  eva_xml_value_reader_set_error (reader,
				  error,
				  error_code,
				  "got %s; expected %s",
				  got, expected);
		      ...
		      GError              **error)
{
  ...
  switch (top->state)
      ...
      case STATE_VALUE:
	{
	  ...
	  type = eva_gtype_loader_load_type (reader->type_loader,
					     element_name,
					     &tmp_error);
	  ...
	  if (!eva_gtype_loader_test_type (reader->type_loader, type))
	    {
	      eva_xml_value_reader_set_error (reader,
					      error,
					      0, /* TODO: error_code */
...
/* XXX: do we really handle g_value_type_transformable? */
  ...
  eva_xml_value_reader_set_error_mismatch (reader,
					   error,
					   0, /* TODO: error_code */
					   "tag <%s>",
					   element_name);
      ...
      case STATE_OBJECT_BODY:
	{
	  ...
	  if (strcmp (element_name, class_name) == 0)
	      ...
	      if (object == NULL)
		{
	      ...
	      eva_xml_value_reader_pop_value (reader);
	      return;
	    }
	}
	break;
...
/* XXX: this isn't reentrant... */
  ...
  if (reader->type_loader)
    {
      eva_gtype_loader_unref (reader->type_loader);
			    ...
			    GError           **error)
{
  ...
  if (reader->parse_context == NULL)
      ...
      for (;;)
	{
      ...
      eva_xml_value_reader_create_parser (reader);
    }
			       ...
			       GError         **error)
{
  ...
  xml_value_reader = eva_xml_value_reader_new (type_loader,
					       object_type,
					       set_object_ptr,
					       &object,
					       NULL);
  ...
  if (!eva_xml_value_reader_input (xml_value_reader,
				   file_contents,
				   file_contents_len,
				   error))
    goto FAIL;
  ...
  eva_xml_value_reader_free (xml_value_reader);
