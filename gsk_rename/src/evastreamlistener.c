...
#define EVA_DEBUG_LISTENER(args) _EVA_DEBUG_PRINTF(EVA_DEBUG_STREAM_LISTENER,args)

/* --- public api --- */
/**
 * eva_stream_listener_handle_accept:
				     ...
				     GDestroyNotify     destroy)
{
  ...
  EVA_DEBUG_LISTENER (("eva_stream_listener_handle_accept: %s[%p] func=%p",
...
/* --- protected api --- */
/**
 * eva_stream_listener_notify_accepted:
				     ...
				     EvaStream         *new_stream)
{
  ...
  EVA_DEBUG_LISTENER (("eva_stream_listener_notify_accepted: %s[%p] received %s[%p]",
  ...
  if (!stream_listener->accept_func (new_stream, stream_listener->data, &error))
	...
	error = g_error_new (EVA_G_ERROR_DOMAIN,
			     ...
			     g_type_name (G_OBJECT_TYPE (stream_listener)));
      eva_stream_listener_notify_error (stream_listener, error);
    }
}

/**
 * eva_stream_listener_notify_error:
...
eva_stream_listener_class_init (EvaStreamListenerClass *class)
{
  ...
  object_class->finalize = eva_stream_listener_finalize;
}

GType eva_stream_listener_get_type()
{
