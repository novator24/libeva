...
int main(int argc, char **argv)
{
  ...
  /* test eva_pass_fd_make_pair() mode */
  for (count = 0; count < 3; count++)
    {
      ...
      if (!eva_pass_fd_make_pair (&send_fd, &receive_fd, &error))
	g_error ("error making pair of file-descriptors: %s", error->message);
	  ...
	  char buf[7];
          ...
          guint8 *msg;
	  close (send_fd);
	  eva_fd_clear_nonblocking (receive_fd);
	  retry:
	  fd = eva_pass_fd_receive (receive_fd, &msg_len, &msg, &error);
	  if (fd < 0)
	    {
	      if (error == NULL)
	        goto retry;
	  ...
	  eva_fd_clear_nonblocking (fd);
	  if (eva_readn (fd, buf, 7) != 7
	   || memcmp (buf, "hi mom\n", 7) != 0)
	...
	g_error ("error running pipe: %s", g_strerror (errno));
      close (receive_fd);
      eva_fd_clear_nonblocking (send_fd);

      /* XXX: race condition */
      sleep(1);

      if (!eva_pass_fd_send (send_fd, pipe_fds[0], AUX_DATA_LEN, AUX_DATA, &error))
        g_error ("eva_pass_fd_send failed: %s",
	         error ? error->message : "transient error");
      close (pipe_fds[0]);
      
      if (eva_writen (pipe_fds[1], "hi mom\n", 7) != 7)
