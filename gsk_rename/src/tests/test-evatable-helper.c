  ...
  while (n < n_data)
    {
      ...
      if (*line == '.')
      ...
      else
        {
          ...
          if (n + 1 == n_data)
            {
              data[n].data = eva_unescape_memory_hex (line, -1, &tmp_size, error);
              if (data[n].data == NULL)
                return FALSE;
              data[n].len = tmp_size;
              line = strchr (line, 0);
          ...
          else
            {
              ...
              data[n].data = eva_unescape_memory_hex (line, end - line, &tmp_size, error);
              if (data[n].data == NULL)
                return FALSE;
              data[n].len = tmp_size;
              line = end;
...
int main(int argc, char **argv)
{
  ...
  context = g_option_context_new ("test-evatable-prog");
  ...
  if (create)
    new_flags = EVA_TABLE_MAY_CREATE;
  else if (existing)
    new_flags = EVA_TABLE_MAY_EXIST;

  options = eva_table_options_new ();
  if (strcmp (op_mode, "replacement") == 0)
    {
      eva_table_options_set_replacement_semantics (options);
  ...
  table = eva_table_new (dir, options, new_flags, &error);
  if (table == NULL)
    g_error ("eva_table_new() failed: %s", error->message);
  ...
  if (strcmp (io_mode, "cmd_prefixed_hex") == 0)
    {
      guint lineno = 1;
      char *line;
      while ((line=eva_stdio_readline (input_fp)) != NULL)
        {
          Data data[2];
          switch (line[0])
            {
            ...
            case 'Q':           /* query */
              {
                ...
                if (!eva_table_query (table, data[0].len, data[0].data,
                                      &found, &value_len, &value_data,
                                      &error))
                  g_error ("eva_table_query failed: %s", error->message);
                if (found)
                  {
                    hex = eva_escape_memory_hex (value_data, value_len);
                    printf ("%s\n", hex);
                    g_free (hex);
                    g_free (value_data);
                  }
            ...
            case 'A':           /* add */
              if (!parse_hex (line+1, 2, data, &error))
                g_error ("error line %u parsing add binary data: %s",
                         lineno, error->message);
              if (!eva_table_add (table, data[0].len, data[0].data,
                                  data[1].len, data[1].data, &error))
                g_error ("eva_table_add failed: %s", error->message);
              g_free (data[0].data);
              g_free (data[1].data);
              break;
            case '!':           /* assert */
