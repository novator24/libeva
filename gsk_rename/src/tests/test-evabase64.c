...
{
  char *encoded;
  guint encoded_len = EVA_BASE64_GET_ENCODED_LEN (len);
  char *out;
  GByteArray *array;
  ...
  eva_base64_encode (encoded, data, len);
  {
    char *out2 = eva_base64_encode_alloc (data, len);
    g_assert (strlen (out2) == encoded_len);
    g_assert (memcmp (out2, encoded, encoded_len) == 0);
    g_free (out2);
  }
  g_assert (eva_base64_decode (out, len, encoded, -1) == len);
  g_assert (memcmp (data, out, len) == 0);
  g_assert (out[len] == (char) 129);
  array = eva_base64_decode_alloc (encoded);
