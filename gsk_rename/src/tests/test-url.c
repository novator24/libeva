...
main (int argc, char **argv)
{
  ...
  url = eva_url_new ("http://wigwam.whatever/path", &error);
  if (url == NULL)
    g_error ("error parsing url: %s", error->message);
  g_assert (url->scheme == EVA_URL_SCHEME_HTTP);
  ...
  g_value_unset(&value);
  str = eva_url_to_string (url);
  g_assert (strcmp (str, "http://wigwam.whatever/path") == 0);
  g_free (str);
  g_object_unref (url);

  url = eva_url_new ("file:///yes/this/might/be/a/path", &error);
  if (url == NULL)
    g_error ("error parsing url: %s", error->message);
  g_assert (url->scheme == EVA_URL_SCHEME_FILE);
  g_assert (url->host == NULL);
  g_assert (strcmp (url->path, "/yes/this/might/be/a/path") == 0);
  g_object_unref (url);

  url = eva_url_new ("http://daveb@host.com/whatever", &error);
  g_assert (url != NULL);
  g_assert (url->scheme == EVA_URL_SCHEME_HTTP);
  ...
  url = eva_url_new ("http://daveb:passwd@host.com/whatever2", &error);
  g_assert (url != NULL);
  g_assert (url->scheme == EVA_URL_SCHEME_HTTP);
  g_assert (strcmp (url->user_name, "daveb") == 0);
  ...
  url0 = eva_url_new ("http://www.time.com/time/time100/index.html", &error);
  g_assert (url0 != NULL);
  g_assert (strcmp (url0->host, "www.time.com") == 0);
  g_assert (strcmp (url0->path, "/time/time100/index.html") == 0);
  url1 = eva_url_new_relative (url0, "https://secure.customersvc.com/servlet/Show?WESPAGE=td/home.html&MSRSM\nAG=TD", &error);
  if (url1 == NULL)
    {
      /* ok */
    }
  ...
  url0 = eva_url_new ("http://www.ecookbooks.com/robots.txt", &error);
  g_assert (strcmp (url0->host, "www.ecookbooks.com") == 0);
  g_assert (strcmp (url0->path, "/robots.txt") == 0);
  url1 = eva_url_new_relative (url0, "chefs.html?#urlparam#", &error);
  if (url1)
   {
  ...
  url0 = eva_url_new ("http://censored.com/index.aspx", &error);
  g_assert (url0 != NULL);
  url1 = eva_url_new_relative (url0, "Article.aspx?ArticleId=35", &error);
  g_assert (url1 != NULL);
  url2 = eva_url_new_relative (url1, "Article.aspx?ArticleId=6", &error);
  g_assert (url2 != NULL);
...
#define _TEST_ENCDEC(test, test_encoded, suffix)					\
  G_STMT_START{								\
    ...
    char *encoded = eva_url_encode##suffix (orig);			\
    char *decoded = eva_url_decode##suffix (encoded);			\
