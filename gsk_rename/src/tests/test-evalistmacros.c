...
#define GET_STACK() Node*,first,next
  g_assert (EVA_STACK_IS_EMPTY (GET_STACK()));
  EVA_STACK_PUSH (GET_STACK (), node_alloc ("a"));
  EVA_STACK_PUSH (GET_STACK (), node_alloc ("b"));
  EVA_STACK_PUSH (GET_STACK (), node_alloc ("c"));
  ...
  EVA_STACK_GET_BOTTOM (GET_STACK (), n);
  g_assert (strcmp (n->str, "a") == 0);
  EVA_STACK_REVERSE (GET_STACK ());
  g_assert (strcmp (first->str, "a") == 0);
  g_assert (strcmp (first->next->str, "b") == 0);
  g_assert (strcmp (first->next->next->str, "c") == 0);
  g_assert (first->next->next->next == NULL);
  EVA_STACK_POP (GET_STACK (), n);
  g_assert (strcmp (n->str, "a") == 0);
  g_assert (strcmp (first->str, "b") == 0);
  EVA_STACK_INSERT_AFTER (GET_STACK (), first, n);
  EVA_STACK_SORT (GET_STACK (), NODE_COMPARE);
  EVA_STACK_FOREACH (GET_STACK (), n, node_free (n));
  first = last = NULL;
#undef GET_STACK

  /* Test QUEUE stuff */
#define GET_QUEUE() Node*,first,last,next
  g_assert (EVA_QUEUE_IS_EMPTY (GET_QUEUE()));
  EVA_QUEUE_PREPEND (GET_QUEUE (), node_alloc ("a"));
  EVA_QUEUE_PREPEND (GET_QUEUE (), node_alloc ("b"));
  EVA_QUEUE_PREPEND (GET_QUEUE (), node_alloc ("c"));
  g_assert (strcmp (first->str, "c") == 0);
  g_assert (strcmp (first->next->str, "b") == 0);
  g_assert (strcmp (first->next->next->str, "a") == 0);
  g_assert (first->next->next->next == NULL);
  g_assert (last == first->next->next);
  EVA_QUEUE_REVERSE (GET_QUEUE ());
  g_assert (strcmp (first->str, "a") == 0);
  g_assert (strcmp (first->next->str, "b") == 0);
  g_assert (strcmp (first->next->next->str, "c") == 0);
  g_assert (first->next->next->next == NULL);
  EVA_QUEUE_DEQUEUE (GET_QUEUE (), n);
  g_assert (strcmp (n->str, "a") == 0);
  g_assert (strcmp (first->str, "b") == 0);
  first = last = NULL;
...
#define GET_STACK() IntNode*,nodes[0],next
    ...
    nodes[i]->next = NULL;
    EVA_STACK_SORT (GET_STACK (), INT_NODE_COMPARE);
    at = nodes[0];
    for (i = 0; i < n_nodes; i++)
      {
        g_assert (at->i == i);
...
#define GET_STACK() IntNode*,nodes[0],next
    ...
    EVA_STACK_SORT (GET_STACK (), INT_NODE_COMPARE_HIGH16);
