...
G_DEFINE_TYPE(EvaUrlTransferHttp, eva_url_transfer_http, EVA_TYPE_URL_TRANSFER);

/* used to restart a transfer on a new URL (a redirect) */
static void start_name_resolution (EvaUrlTransferHttp *http);

struct _EvaUrlTransferHttpModifierNode
{
  EvaUrlTransferHttpRequestModifierFunc modifier;
  gpointer data;
  GDestroyNotify destroy;
  EvaUrlTransferHttpModifierNode *next;
};
...
handle_http_response (EvaHttpRequest  *request,
                      ...
                      gpointer         hook_data)
{
  EvaUrlTransfer *transfer = EVA_URL_TRANSFER (hook_data);
  EvaUrlTransferHttp *http = EVA_URL_TRANSFER_HTTP (hook_data);
  ...
  /* For exposition we retain copy
     the entire list of EvaHttpStatusCodes.
  ...
  switch (response->status_code)
	...
	if (response->location != NULL)
	  {
	    EvaUrl *old_url = transfer->url;
	    GError *error = NULL;
	    new_url = eva_url_new_relative (old_url, response->location, &error);
	    if (new_url == NULL)
	      {
                EvaUrlTransferResult result = EVA_URL_TRANSFER_ERROR_UNSUPPORTED;       /* XXX: not especially correct */
...
http_client_request_destroyed (gpointer data)
{
  ...
  EvaUrlTransferHttp *http = EVA_URL_TRANSFER_HTTP (data);
...
static void
handle_name_resolution_succeeded (EvaSocketAddress *address,
                                  gpointer          data)
{
  EvaUrlTransfer *transfer = EVA_URL_TRANSFER (data);
  EvaUrlTransferHttp *http = EVA_URL_TRANSFER_HTTP (data);
  ...
  EvaStream *upload_stream;
  EvaUrlTransferHttpModifierNode *modifier;
  EvaUrl *url = transfer->redirect_url ? transfer->redirect_url : transfer->url;
  ...
  {
    EvaSocketAddressIpv4 *found = EVA_SOCKET_ADDRESS_IPV4 (address);
...
static void
start_name_resolution (EvaUrlTransferHttp *http)
{
  EvaUrlTransfer *transfer = EVA_URL_TRANSFER (http);
  EvaUrl *url = transfer->redirect_url ? transfer->redirect_url : transfer->url;
...
static gboolean
eva_url_transfer_http_start (EvaUrlTransfer *transfer,
                             GError        **error)
{
  EvaUrlTransferHttp *http = EVA_URL_TRANSFER_HTTP (transfer);
  EvaUrl *url = transfer->url;
  if (url->host == NULL)
    {
      g_set_error (error,
                   EVA_G_ERROR_DOMAIN,
...
static void
cancel_internal (EvaUrlTransferHttp *http)
{
  if (http->name_lookup)
    {
  ...
  else if (http->raw_transport)
    {
      ...
      if (error)
        {
          g_message ("EvaUrlTransferHttp: error shutting down for cancellation: %s", error->message);
          g_error_free (error);
        }
    }
}
...
static void
eva_url_transfer_http_cancel (EvaUrlTransfer *transfer)
{
  g_object_ref (transfer);
...
static void
eva_url_transfer_http_timed_out (EvaUrlTransfer *transfer)
{
  cancel_internal (EVA_URL_TRANSFER_HTTP (transfer));
...
static char *
eva_url_transfer_http_get_running_state (EvaUrlTransfer *transfer)
{
...
{
  EvaUrlTransferHttp *http = EVA_URL_TRANSFER_HTTP (object);
  EvaUrlTransferHttpModifierNode *mod;
  ...
  for (mod = http->first_modifier; mod; )
    {
      EvaUrlTransferHttpModifierNode *next = mod->next;
      if (mod->destroy)
...
static void
eva_url_transfer_http_init (EvaUrlTransferHttp *url_transfer_http)
{
}

static void
eva_url_transfer_http_class_init (EvaUrlTransferHttpClass *class)
{
 ...
 */
void
eva_url_transfer_http_set_ssl_cert    (EvaUrlTransferHttp *http,
...
void
eva_url_transfer_http_set_ssl_key     (EvaUrlTransferHttp *http,
...
void
eva_url_transfer_http_set_ssl_password(EvaUrlTransferHttp *http,
                                       const char         *password)
{
...
static void
transfer_modifier_set_user_agent (EvaHttpRequest *request,
                                  gpointer        mod_data)
{
 ...
 */
void
eva_url_transfer_http_set_user_agent  (EvaUrlTransferHttp *http,
 ...
 */
void
eva_url_transfer_http_set_proxy_address  (EvaUrlTransferHttp *http,
...
static void
transfer_modifier_set_misc_header  (EvaHttpRequest *request,
                                    gpointer        mod_data)
{
  char *key = mod_data;
 ...
 */
void
eva_url_transfer_http_add_extra_header(EvaUrlTransferHttp *http,
                                       const char         *key,
                                       const char         *value)
{
 ...
 */
void
eva_url_transfer_http_add_modifier (EvaUrlTransferHttp *http,
                                    EvaUrlTransferHttpRequestModifierFunc modifier,
                                    gpointer data,
                                    GDestroyNotify destroy)
{
  EvaUrlTransferHttpModifierNode *node = g_new (EvaUrlTransferHttpModifierNode, 1);
