...
#if HAVE_EXECINFO_H
#include <execinfo.h>
#define eva_backtrace(contexts, max_levels)         backtrace(contexts,max_levels)
#define eva_backtrace_symbols(contexts, n_contexts) backtrace_symbols(contexts, n_contexts) 
#else
/* TODO: need more useful implementation on other systems... */
static guint
eva_backtrace (gpointer *contexts, guint n_contexts)
{
  return 0;
}
static char **
eva_backtrace_symbols(gpointer *contexts, guint n_contexts)
{
  char **rv;
  assert (n_contexts == 0);
...
void eva_debug_alloc_open_log (const char *output)
{
  ...
  if (log_fd < 0)
    {
      g_error ("eva_debug_alloc_open_log: failed!!!: %s", g_strerror (errno));
    }
  else
    {
      time_t t = time(NULL);
...
static gboolean add_time_to_log (gpointer data)
{
  ...
  log_uint(EVA_MAIN_LOOP(data)->current_time.tv_sec);
  return TRUE;
}
...
debug_malloc      (gsize    n_bytes)
{
  ...
  guint n_levels = eva_backtrace (context, total_levels);
...
resolve_executable_symbols (guint n, char **symbols, gpointer *to_free_out)
{
  ...
  snprintf (fname, sizeof (fname),
            "/tmp/eva-debug-memdump.tmp.%lu.%u.%u",
...
void eva_print_debug_mem_vtable (void)
{
  ...
  /* get the symbols */
  symbols = eva_backtrace_symbols (code_contexts, n_nonempty_contexts);
                                   ...
                                   GDestroyNotify destroy)
{
  ...
  td->destroy = destroy;
  td->object = object;
  td->source = eva_main_loop_add_timer (eva_main_loop_default (),
