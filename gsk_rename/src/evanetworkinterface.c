...
eva_network_interface_set_new(EvaNetworkInterfaceFlags  flags)
{
  ...
  if (tmp_socket < 0)
    {
      g_warning ("eva_network_interface: error creating internal ns socket: %s",
		 g_strerror (errno));
      return NULL;
    }
  ...
  for (;;)
	...
	{ 
	  g_warning ("eva_network_interface:"
  ...
  for (i = 0; i < ifreq_array->len; i++)
	...
	{	
      ...
      if ((flags & EVA_NETWORK_INTERFACE_UP) != 0 && !is_up)
	continue;
      if ((flags & EVA_NETWORK_INTERFACE_LOOPBACK) != 0 && !is_loopback)
	continue;
      if ((flags & EVA_NETWORK_INTERFACE_NON_LOOPBACK) != 0 && is_loopback)
	continue;
      if ((flags & EVA_NETWORK_INTERFACE_HAS_BROADCAST) != 0 && !has_broadcast)
	continue;
      if ((flags & EVA_NETWORK_INTERFACE_HAS_MULTICAST) != 0 && !has_multicast)
	continue;
      ...
      if (is_up)
	{
	  ...
	  interface.address = eva_socket_address_from_native (saddr, sizeof (*saddr));
	}
      else
      ...
      else
	{
	  ...
	  interface.hw_address = eva_socket_address_ethernet_new ((guint8*)tmp_req.ifr_addr.sa_data);
	}
#else
      interface.hw_address = NULL;
#endif
      ...
      if (is_p2p)
	{
	  ...
	  saddr = &tmp_req.ifr_addr;
	  interface.p2p_address = eva_socket_address_from_native (saddr,
      ...
      if (has_broadcast)
	{
	  ...
	  saddr = &tmp_req.ifr_addr;
          interface.broadcast = eva_socket_address_from_native (saddr,
								sizeof (struct sockaddr));
	}
      else
...
/**
 * eva_network_interface_set_destroy:
