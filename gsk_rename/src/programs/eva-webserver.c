...
usage (void)
{
  g_printerr ("eva-webserver OPTIONS\n\n"
              "OPTIONS include:\n");
  ...
  for (i = 1; i < (guint)argc; i++)
    {
	  ...
	  guint port = atoi (strchr (argv[i], '=') + 1);
      ...
      else if (strcmp (argv[i], "--mime") == 0)
        {
          ...
          eva_http_content_set_mime_type (content, prefix, suffix, type, subtype);
          g_free (prefix);
          g_free (suffix);
          g_free (type);
          g_free (subtype);
      ...
      else if (strcmp (argv[i], "--default-mime") == 0)
        {
          ...
          type = g_strndup (type_subtype, strchr (type_subtype, '/') - type_subtype);
          subtype = g_strdup (strchr (type_subtype, '/') + 1);
          eva_http_content_set_default_mime_type (content, type, subtype);
          g_free (type);
          g_free (subtype);
        }
      else if (strcmp (argv[i], "--location") == 0)
        {
          ...
          eva_http_content_add_file (content, url_path, fs_path,
